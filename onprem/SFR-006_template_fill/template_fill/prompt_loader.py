...
`TEMPLATE_FILL_PROMPT_SOURCE` 로 고른다:
- `file`(기본) — `onprem/prompt/SFR-006_template_fill/<이름>.j2`
...
- `prompt_id` 는 GenOS 가 발급하는 정수다. 코드에 박지 않고
  `TEMPLATE_FILL_PROMPT_ID_<이름 대문자>` 환경변수로 받는다 (§10.5 금지 조항).
...
`TEMPLATE_FILL_PROMPT_TTL_SECONDS`(기본 300) 동안 본문을 들고 있는다 — 리비전을 운영에
...
def source() -> str:
    ...
    value = os.environ.get("TEMPLATE_FILL_PROMPT_SOURCE", SOURCE_FILE).strip().lower()
...
def _ttl_seconds() -> float:
    try:
        return max(0.0, float(os.environ.get("TEMPLATE_FILL_PROMPT_TTL_SECONDS", "300")))
    except ValueError:
        return 300.0


def _prompt_id(name: str) -> str:
    """논리 이름 → `TEMPLATE_FILL_PROMPT_ID_<이름 대문자>` 환경변수 값."""
    return os.environ.get(f"TEMPLATE_FILL_PROMPT_ID_{name.upper()}", "").strip()
