...
def list_policies_for_admin_ui() -> list[dict]:
    """매니페스트 기반 관리자 UI에 그대로 넘길 수 있는 형태."""
    return [
        {
            "value": key,
            "label": p.label,
            "forced_tone": p.forced_tone,
            "allowed_tones": list(p.allowed_tones) if not p.forced_tone else [p.forced_tone],
        }
        for key, p in DOC_TYPE_POLICIES.items()
    ]
