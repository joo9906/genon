...
**글다듬이는 문서를 출력하지 않는다** — 채팅 응답으로 끝난다.
...
**문서 출력은 하지 않는다.** 원본을 `source_markdown` 으로 함께 낸다.
...
| 경로 | 하는 일 |
|---|---|
| `GET /config` | 상한·기본 개수·**내려받을 수 있는 형식** (UI 가 버튼을 켠다) |
| `POST /generate` | 마크다운 본문으로 생성 |
| `POST /generate/upload` | **hwpx 업로드 직접 파싱** 후 생성 |
| `GET /faqs` | 세션에 저장된 FAQ 조회 |
| `POST /download` | hwpx / pdf / xlsx |

### 4-4. 내려받기

- **저장된 것을 내려준다. 다시 생성하지 않는다** — LLM 을 다시 부르면 화면에서 본
  FAQ 와 파일이 달라진다. **다운로드가 세션을 지우지 않는다**(형식만 바꿔 여러 번 받는
  흐름이 정상이라 006 과 다르다).
- **hwpx 는 템플릿 기반**이다. 백지에서 조립하면 `header.xml` 한 글자 차이로 한/글이
  문서를 못 여는데 확인할 한/글이 없다. 관리자 템플릿의 반복 블록
  (`{{question}}`/`{{answer}}`/`{{evidence}}`)을 복제한다. **템플릿 미등록 시 501**.
- pdf 는 두 경로: hwpx 템플릿이 있으면 **사내 서식 그대로**(hwpx→pdf), 없으면 마크다운
  렌더링.
- xlsx 는 **수식 인젝션 방지**를 한다.
- 가용성은 프로세스당 1회 판별해 `/config` 로 알린다 — 못 만드는 형식의 버튼을 켜두면
  사용자는 눌러 보고서야 501 을 받는다.
