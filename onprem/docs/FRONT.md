...
## 요약 — 프론트가 받는 것은 이게 전부다 (2026-09-08 확정)
...
**모든 기능 공통으로 함께 올 수 있는 것 셋** (그리고 이게 전부다):

| 키 | 언제 | 왜 |
|---|---|---|
| `genos_state` | 플랫폼이 넣을 때 | GenOS 추적(`trace_id`). **화면은 안 읽는다** — 그대로 두면 된다 |
| `notice` | 있을 때만 (글다듬이·번역·FAQ) | 문자열 배열. **그대로 보여준다** (§2.3·§3.3) |
...
> **006 은 `notice` 가 없다** — 안내가 `text` 문장에 이미 들어 있다. `error: null` 을
> 싣던 예외도 2026-09-08 에 없어졌다(네 기능이 같은 규약이 됐다).
...
| 변수 | 필수? | 값 |
|---|---|---|
| `genosUploaded` | ✅ | 전처리기 산출물. **사용자가 파일을 올리면 캔버스가 채운다** |
...
네 기능이 같은 네 갈래를 쓴다. `msg` 를 그대로 보여주면 되고, 화면이 분기할 값은
**`retryable`** 하나다.
...
6. **006 의 `download_url` 은 항목을 다 채운 뒤에만 온다** (§4.4). 그 전에는 `null` 이라
   버튼을 꺼 두면 되는데, **"아직 다 안 채웠다" 와 "업로드가 실패했다" 를 화면이 구분할
   수 없다.** 지금은 둘 다 `null` 이다 — 구분이 필요하면 알려 달라(백엔드가 사유를
   `notice` 로 얹을 수 있다).
