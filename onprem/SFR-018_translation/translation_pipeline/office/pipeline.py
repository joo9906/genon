...
[설계]
1. trans_map은 참고용이다. 같은 원문이 문서에 여러 번 등장하면(예: 반복 머리글)
   dict 키가 충돌해 마지막 번역으로 수렴하므로, 위치 정확도가 필요한 소비자는
   반드시 pairs(node_id 기준)를 쓴다.
2. TranslationRequestError에는 우리가 작성한 고정 안내문만 담는다
   (main.py가 이 메시지를 API 응답 msg로 그대로 사용하기 때문).
"""
...
async def run_translation_job(
    *,
    ...
    translated_by_unit_id, translation_error = await translate_units(
        sem,
        units,
        target_lang,
...
async def run_markdown_translation_job(
    *,
    ...
    """전처리기 산출물(마크다운/HTML 표)을 구조 보존 방식으로 번역한다.
