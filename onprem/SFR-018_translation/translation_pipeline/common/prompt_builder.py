...
def get_translation_system_prompt(target_lang: str, style_options: Dict[str, Any] | None = None) -> str:
    return (
        ...
        "Preserve any inline markdown syntax inside items (e.g. **bold**, `code`, [links](url)) "
        "and keep numbers, dates and proper nouns unchanged."
    )


def get_single_translation_system_prompt(target_lang: str) -> str:
    return (
        f"Translate the given text into {target_lang}. Return only the translated text. "
        "Preserve any inline markdown syntax (e.g. **bold**, `code`, [links](url))."
    )
