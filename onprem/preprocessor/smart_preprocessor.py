"""GenOS 통합 전처리기 — **지능형(pdf·docx·ppt·엑셀·이미지) + 우리 hwpx 파서**.

> **등록 단위 하나짜리 파일이다.** GenOS 전처리기 등록에 이 파일을 통째로 올리면
> 확장자에 따라 두 처리기 중 하나로 간다. 진입점은 맨 아래 `DocumentProcessor` 다.

## `final_preprocessor.py` 와 무엇이 다른가

**벤더 절반이 다르다. 그것뿐이다.**

| | `final_preprocessor.py` | **이 파일** |
|---|---|---|
| 벤더 절반 | 첨부용(`attach_processor.py`) | **지능형**(`intelligence_processor.py`) |
| pdf | `PyMuPDFLoader` 평문 + 문자 수 분할 → **표가 사라진다** | **docling layout + TableFormer + OCR** |
| ppt·엑셀·이미지 | 첨부용 | 지능형 |
| `.hwp`·`.hml`·오디오 | 첨부용 네이티브(GenosHwp SDK·Whisper) | **없다** (아래 "잃는 것") |
| hwpx | 우리 파서 (같은 코드) | 우리 파서 (같은 코드) |

**둘 중 하나만 등록한다.** 같은 컬렉션에 둘을 함께 걸면 같은 pdf 가 등록 시점에 따라
다른 청크로 들어가고, 그 어긋남은 오류가 아니라 검색 품질로만 드러난다.

## 왜 다시 만들었나

지능형은 2026-09-01 에 걷어냈었다 — 사이트 설치본 판본과 어긋나 pdf 적재가
`ModuleNotFoundError` 로 막혔기 때문이다. **2026-09-08 에 지능형이 실환경에서 돈다는
것이 확인돼** 다시 짝지었다. 그때 감수했던 것("첨부용 pdf 는 표가 통째로 사라진다")이
이 파일의 존재 이유다.

## 잃는 것 — 첨부용에만 있던 세 경로

지능형에는 **`.hwp`(구버전 바이너리)·`.hml`·오디오 경로가 없다.** 첨부용은 그 셋을
네이티브로 읽었다(GenosHwp SDK · Whisper STT). 이 파일로 등록하면 그 확장자는
지능형의 일반 경로로 가고, **`.hwp` 는 PDF 변환을 거치거나 아예 실패한다.**

- 그 형식이 필요하면 **`final_preprocessor.py` 를 쓰거나**, 등록 화면에서 확장자를
  갈라 두 등록을 둔다(그때는 컬렉션을 나눈다 — 위 "둘 중 하나만" 참고).
- **조용히 빠지지 않는다**: 라우터가 그 확장자를 만나면 로그에 남긴다
  (`event=smart_preprocess_vendor_gap`).

## 합친 순서가 계약이다

**지능형 → hwpx → 라우터** 순으로 붙어 있고 **뒤엣것이 앞엣것을 덮는다.** 그래서
겹치는 최상위 이름을 미리 갈랐다 — 실제 충돌은 **둘뿐**이었다(나머지 여섯은 `os`·`re`·
`time`·`logging`·`datetime`·`Any` 로, 같은 표준 모듈이라 덮여도 같다):

| 원래 이름 | 바꾼 이름 | 왜 |
|---|---|---|
| `DocumentProcessor` (지능형) | `IntelligentDocumentProcessor` | **진입점 이름**이다. 안 바꾸면 라우터가 덮여 hwpx 가 영영 안 탄다 |
| `_log` (지능형) | `_intel_log` | 로거가 뭉치면 어느 절반이 낸 로그인지 로그에서 사라진다 |

**개명은 토큰 단위로 했다.** 정규식으로 갈면 지능형의 `[DocumentProcessor]` 로 시작하는
로그 문자열까지 바뀐다(MCP 넷을 합칠 때 `resolve_tone` 에서 실제로 밟은 함정).
`onprem/test/check_smart_preprocessor.py` 가 참조 원본에서 다시 계산해 대조한다 —
GenOS 가 다음 릴리스에서 지능형을 고치면 그 자리에서 잡힌다.

## 지능형 절반은 통째로 `try` 안에 있다

docling 스택이 없는 환경에서도 이 파일이 import 되고 **hwpx 경로가 살아 있어야** 한다.
무거운 의존 하나가 빠졌을 때 hwpx 적재까지 같이 죽으면 안 되고, 회귀 점검이 로컬에서
이 파일을 태울 수 있어야 한다. **실패 사실은 숨기지 않는다** — 라우터가 그대로 드러낸다
(`_sp_engine_error`). 2026-09-02 에 사이트 설치본에 없는 모듈 하나 때문에 첨부용 절반이
통째로 가드에 걸려 **"pdf 만 안 되는"** 얼굴이 됐던 적이 있다.

## 스키마 — **UI 에서 누락되지 않게 하는 것이 요점이다**

한 등록이 한 컬렉션에 **두 모양의 메타**를 넣으면, 그 필드로 거르는 검색이 한쪽을 통째로
놓친다. 오류가 아니라 "결과가 좀 적네" 로만 보인다. 그리고 적재 결과 **화면**은 청크를
`i_page` 로 묶어 그리므로, 그 필드가 비면 hwpx 로 넣은 문서만 화면에 안 뜬다.

그래서 두 가지를 한다:

1. **페이지 자리에 구역(section)을 넣는다** (`_page_fields`) — 1-based `i_page`,
   0-based `i_chunk_on_page`, 빈 좌표는 `"[]"`, 미디어 없음은 `""`. 값의 출처는
   `page_basis` 가 말한다("이 숫자는 렌더링된 페이지가 아니라 구역이다").
2. **벤더 예약 필드를 채운다** (`_sp_align_records`) — 목록을 손으로 적지 않고
   **지능형 `GenOSVectorMeta` 에서 뽑는다.** 벤더가 필드를 늘리면 따라간다.
   벤더 절반이 안 떴을 때를 대비해 baseline 을 함께 둔다(그때도 정렬은 돼야 한다).

값은 **지어내지 않는다** — 벤더가 못 채웠을 때 내는 것과 같은 것을 쓴다.

## 아직 안 한 것

**조/항/호 위계를 지능형 경로에 걸지 않았다.** `final_preprocessor.py` 는 벤더 청커를
갈아 끼워 pdf·docx 에도 조문 위계를 매기는데(`_fp_enable_outline`), 그 배선은 첨부용
산출물 모양(langchain `Document` 목록)을 전제로 만든 것이라 지능형(`DoclingDocument` +
`GenosSmartChunker`)에 그대로 얹을 수 없다. 잘못 걸면 **매 요청 실패한 뒤 벤더 청커로
돌아가 위계가 영영 안 붙는데 결과는 정상으로 보인다.** hwpx 의 조문 위계는 그대로다.
"""
...
# ===========================================================================
# PART 1 — 지능형 (벤더). 참조 원본: genos_files/intelligence_processor.py
# ===========================================================================
# 통째로 try 안에 있다 — 근거는 파일 맨 위 "합친 순서가 계약이다" 절.
_SP_INTEL_IMPORT_ERROR = None
_SP_INTEL_IMPORT_TRACE = ""
try:
    ...
    _intel_log = logging.getLogger(__name__)
    ...
    """
        try:
            ...
            _intel_log.warning(f"[_has_any_pdf_converter] PDF 변환기 가용성 확인 실패: {exc}")
            return True
    ...
    def _parse_optional_bool(value: Any, key: str = "") -> Optional[bool]:
        ...
        if isinstance(value, str):
            ...
            if text in {"0", "false", "no", "n", "off"}:
                return False
        if key:
            _intel_log.warning(f"[DocumentProcessor] Invalid bool value for '{key}': {value!r}. Fallback to default.")
        return None
    ...
    def _parse_optional_int(value: Any, key: str = "") -> Optional[int]:
        ...
        try:
            ...
            if key:
                _intel_log.warning(f"[DocumentProcessor] Invalid int value for '{key}': {value!r}. Fallback to default.")
            return None
    ...
    0(=분할 안 함) 과 None 은 그대로 둔다."""
        if size is not None and 0 < size < _MIN_CHUNK_SIZE:
            _intel_log.info(f"[chunk_size] {size} < {_MIN_CHUNK_SIZE} → {_MIN_CHUNK_SIZE} 로 보정")
    ...
    def _parse_optional_float(value: Any, key: str = "") -> Optional[float]:
        ...
        try:
            ...
            if key:
                _intel_log.warning(f"[DocumentProcessor] Invalid float value for '{key}': {value!r}. Fallback to default.")
            return None
    ...
    """
        ...
        _intel_log.warning(f"[DocumentProcessor] {stage} enrichment skipped ({error_type}): {exc}")
    ...
    class GenosSmartChunker(BaseChunker):
        ...
        def _initialize_components(self) -> Self:
            ...
            if mode not in {"char", "huggingface"}:
                _intel_log.warning(f"[GenosSmartChunker] Unknown tokenizer_type '{mode}', fallback to 'char'.")
                mode = "char"
    ...
    class IntelligentDocumentProcessor:
        ...
        '''
            ...
            if self._chunk_mode not in {"split_only", "resize_all"}:
                _intel_log.warning(f"[DocumentProcessor] Unknown chunking.chunk_mode '{self._chunk_mode}', fallback to 'split_only'.")
            ...
            if raw_ocr_mode not in {"auto", "force", "disable"}:
                _intel_log.warning(f"[DocumentProcessor] Unknown ocr_mode '{raw_ocr_mode}', fallback to 'auto'")
                raw_ocr_mode = "auto"
            ...
            if device is None:
                _intel_log.warning(f"[DocumentProcessor] Unknown pdf_pipeline.device '{device_str}', fallback to 'auto'")
        ...
        """
            ...
            if ocr_engine not in {"paddle", "upstage"}:
                _intel_log.warning(f"[DocumentProcessor] Unknown ocr.engine '{ocr_engine}', fallback to 'paddle'")
                ocr_engine = "paddle"
            ...
            if ocr_engine == "upstage":
                ...
                try:
                    ...
                    _intel_log.warning(f"[DocumentProcessor] Invalid ocr.upstage.timeout '{raw_timeout}', fallback to 60")
                    upstage_timeout = 60
                ...
                try:
                    ...
                    _intel_log.warning(f"[DocumentProcessor] Invalid ocr.upstage.text_score '{raw_text_score}', fallback to 0.5")
            ...
            if isinstance(raw_lang, list) and raw_lang:
                ...
                if raw_lang not in (None, [], ["korean"]):
                    _intel_log.warning(f"[DocumentProcessor] Invalid ocr.paddle.lang '{raw_lang}', fallback to ['korean']")
                paddle_lang = ["korean"]
            ...
            try:
                ...
                _intel_log.warning(f"[DocumentProcessor] Invalid ocr.paddle.text_score '{raw_text_score}', fallback to 0.3")
        ...
        """
            ...
            _intel_log.info(f"[ppt] page-based chunks: {len(page_chunks)} (chunk_size={chunk_size})")
            return page_chunks
        ...
        def enrichment(self, document: DoclingDocument, is_ppt: bool = False, **kwargs: dict) -> DoclingDocument:
            ...
            if want_toc != cur_toc:
                if want_toc and not str(getattr(options, "toc_api_base_url", "") or ""):
                    _intel_log.warning("[intelligent] toc=1 요청이지만 TOC endpoint 미구성 → 무시")
                else:
                    options = _copy_enrichment_options(options, do_toc_enrichment=want_toc)
                    _intel_log.info("[intelligent] runtime toc override → %s", want_toc)
            ...
            if is_ppt and getattr(options, "do_toc_enrichment", False):
                ...
                _intel_log.info("[intelligent] PPT — TOC enrichment skip")
            try:
        ...
        """
            ...
            _intel_log.info(f"[page_image_description] 페이지 설명 주입: pages={len(page_descs)}")
            return document
        ...
        """
            ...
            for page_no, n_items in page_item_count.items():
                ...
                if n_items > 0 and page_text_len[page_no] == 0:
                    _intel_log.info(f"[intelligent] page {page_no} 텍스트가 비어있음 → 강제 OCR 필요")
                    return True
            return False
        ...
        def _convert_to_pdf(self, file_path: str, **kwargs: dict) -> tuple[str, str]:
            ...
            _intel_log.info(f"[intelligent] Non-PDF input — auto-converting to PDF: {file_path}")
            ...
            if (not converted or not os.path.exists(converted)) and use_sdk:
                _intel_log.warning(f"[intelligent] SDK conversion failed → fallback to LibreOffice")
            ...
            _intel_log.info(f"[intelligent] Converted PDF: {converted}")
        ...
        """
            ...
            if self._xlsx_cfg["processing_mode"] == "tabular":
                _intel_log.info(f"[intelligent] xlsx tabular 직접 처리: {file_path}")
            ...
            _intel_log.info(f"[intelligent] xlsx docling 직접 처리(PDF 변환 생략): {file_path}")
            try:
        ...
        async def __call__(self, request: Request, file_path: str, **kwargs: dict):
            ...
            try:
                _intel_log.info(f"file_path: {file_path}")
                _intel_log.info(f"kwargs: {kwargs}")
...
except Exception as _sp_exc:  # noqa: BLE001 - 무엇이 빠졌든 hwpx 경로는 살린다
    _SP_INTEL_IMPORT_ERROR = _sp_exc
    _SP_INTEL_IMPORT_TRACE = traceback.format_exc()
...
# ===========================================================================
class SmartPreprocessorError(Exception):
    """라우터 자신이 내는 오류.

    hwpx 파싱 실패는 `HwpxParseError` 로, 지능형 경로의 실패는 그쪽 예외 그대로 올린다 —
...
_SP_HWPX = "hwpx"
_SP_INTEL = "intel"
_SP_ENGINES = (_SP_HWPX, _SP_INTEL)

_SP_ENGINE_AUTO = "auto"
_SP_ENGINE_NATIVE = "native"
_SP_HWPX_ENGINES = (_SP_ENGINE_AUTO, _SP_ENGINE_NATIVE, _SP_INTEL)

_SP_ZIP_MAGIC = b"PK\x03\x04"
_SP_HWPX_MIMETYPE = b"application/hwp+zip"
_SP_SECTION_PREFIX = "Contents/section"

# ---------------------------------------------------------------------------
# 확장자 → 엔진.
#
# hwpx 만 우리 파서로 가고 **나머지는 전부 지능형**이다. 지능형은 확장자별로 자기가
# 갈라 받는다 — pdf 는 docling layout + TableFormer + OCR, ppt 는 PDF 변환 후 docling,
# 엑셀(`.xlsx`·`.xlsm`·`.csv`)은 PDF 변환을 건너뛰고 직접(#288), 이미지는 OCR.
#
# 여기 없는 확장자는 `_SP_DEFAULT_ENGINE` 으로 간다.
# ---------------------------------------------------------------------------
_SP_ROUTES = {
    # 우리 파서 — 표 병합·조문 위계를 지킨다
    ".hwpx": _SP_HWPX,
}
_SP_DEFAULT_ENGINE = _SP_INTEL

# **첨부용에만 있던 경로.** 지능형에는 이 셋을 네이티브로 읽는 코드가 없다 —
# `.hwp`/`.hml` 은 GenosHwp SDK, 오디오는 Whisper STT 로 첨부용이 읽었다.
# 조용히 빠지면 "그 문서만 검색이 이상하다" 로만 드러나므로 **만날 때마다 로그에 남긴다.**
# 그 형식이 필요하면 `final_preprocessor.py` 로 등록한다(파일 맨 위 "잃는 것" 절).
_SP_VENDOR_GAP_EXTENSIONS = (".hwp", ".hml", ".mp3", ".wav", ".m4a", ".flac", ".ogg")
...
_SP_CONFIG_ENV = "GENOS_INTELLIGENT_CONFIG_PATH"
_SP_CONFIG_BASENAMES = ("intelligent_processor_config.yaml",)
_SP_CONFIG_KWARG = "intelligent_config_path"

# 벤더 레코드에는 늘 있고 hwpx 레코드에는 없는 예약 필드의 **기본값**.
#
# **목록을 손으로 적지 않는다** — `_sp_schema_defaults()` 가 지능형 `GenOSVectorMeta` 에서
# 뽑아 이 표와 합친다. 손으로만 적으면 벤더가 필드를 늘렸을 때 그 필드만 hwpx 레코드에서
# 비고, 그 상태는 "검색 결과가 좀 적네" 로만 보인다.
#
# 그래도 baseline 이 필요한 이유: **벤더 절반이 안 떴을 때도 정렬은 돼야 한다.** 그때
# 모델을 못 읽는다고 정렬을 건너뛰면, 지능형이 없는 환경에서 넣은 hwpx 레코드만 모양이
# 달라진다. 값은 벤더가 못 채웠을 때 내는 것과 **같은 것**을 쓴다 — 지어내지 않는다.
_SP_SCHEMA_BASELINE = {
    "title": "",
    "created_date": None,
    "appendix": "",
...
# 우리 hwpx 경로가 **이미 채우는** 필드. 정렬 대상에서 뺀다 — 여기 있는 이름을 벤더
# 모델에서 다시 가져와 `setdefault` 하면 값이 있으니 아무 일도 안 일어나지만, 목록을
# 읽는 사람이 "이건 안 채워지는 필드" 로 오해한다.
_SP_HWPX_OWN_FIELDS = (
    "text", "n_char", "n_word", "n_line",
    "i_page", "e_page", "n_page", "i_chunk_on_page", "n_chunk_of_page",
    "i_chunk_on_doc", "n_chunk_of_doc", "reg_date",
    "chunk_bboxes", "media_files", "file_path",
)


def _sp_schema_defaults() -> dict:
    """정렬해 넣을 `{필드: 기본값}`. **지능형 모델에서 뽑고** baseline 과 합친다.

    벤더 모델을 못 읽는 경우가 둘이다 — 지능형 절반이 `try` 에 걸렸을 때, 그리고 벤더가
    `GenOSVectorMeta` 의 모양을 바꿨을 때. 둘 다 **정렬을 건너뛰지 않는다**(baseline 으로
    떨어진다) — 건너뛰면 그때 넣은 레코드만 모양이 달라지고 나중에 원인을 못 찾는다.

    `pydantic` v2 는 `model_fields`, v1 은 `__fields__` 다. 어느 쪽인지 모르므로 둘 다 본다.
    """
    defaults = dict(_SP_SCHEMA_BASELINE)
    model = globals().get("GenOSVectorMeta")
    fields = getattr(model, "model_fields", None) or getattr(model, "__fields__", None)
    if not fields:
        return defaults
    for name in fields:
        if name in _SP_HWPX_OWN_FIELDS or name in defaults:
            continue
        # 모르는 새 필드는 `None` 으로 둔다. 빈 문자열·0 을 넣으면 "값이 있다" 로 읽힌다.
        defaults[name] = None
    return defaults


def _sp_engine_error(engine: str):
    """그 엔진을 쓸 수 없게 만든 예외. 쓸 수 있으면 `None`."""
    if engine == _SP_INTEL:
        return _SP_INTEL_IMPORT_ERROR
    return None


def _sp_choice_kwarg(value, default: str, allowed: tuple, key: str) -> str:
    ...
    if text in allowed:
        return text
    _log_warning(f"unknown {key} - falling back to '{default}'", event="smart_preprocess_bad_kwarg")
    return default


def _sp_bool_kwarg(value, default: bool, key: str) -> bool:
    if value is None or value == "":
        return default
    if isinstance(value, bool):
        return value
...
def _sp_overrides_kwarg(value) -> dict:
    """`{".pdf": "intel"}` 꼴 라우팅 덮어쓰기. 문자열(JSON)로 와도 받는다."""
    if not value:
        return {}
    if isinstance(value, str):
        try:
            ...
            _log_warning("route_overrides is not valid JSON - ignored",
                         event="smart_preprocess_bad_kwarg")
            return {}
...
def _sp_is_hwpx_container(file_path: str):
    """내용이 hwpx 인가. `(판정, 사유)`.
    ...
    if mimetype == _SP_HWPX_MIMETYPE:
        return True, "mimetype"
    for name in names:
        if name.startswith(_SP_SECTION_PREFIX) and name.endswith(".xml"):
            return True, "section_xml"
    # zip 이긴 한데 hwpx 가 아니다 — docx/pptx/xlsx 가 `.hwpx` 이름을 달고 온 경우다.
    return False, "zip_without_hwpx_contents"


def _sp_route(file_path: str, hwpx_engine: str, overrides: dict):
    """`(엔진, 사유)`. 사유는 고정 문자열이라 로그에 그대로 실어도 된다(3.8절)."""
    extension = os.path.splitext(file_path)[1].lower()
    if extension in _SP_VENDOR_GAP_EXTENSIONS and extension not in overrides:
        # 막지는 않는다 — 지능형이 어떻게든 읽어낼 수도 있고, 여기서 거절하면 그 문서가
        # **검색에서 통째로 사라진다.** 다만 조용히 넘기지도 않는다.
        _log_warning(
            "this extension had a native path only in the attachment processor "
            "(.hwp/.hml via GenosHwp SDK, audio via Whisper) - quality may drop",
            event="smart_preprocess_vendor_gap",
            status=extension,
        )
    engine = overrides.get(extension) or _SP_ROUTES.get(extension, _SP_DEFAULT_ENGINE)
    ...
    # 여기부터는 hwpx 확장자다.
    if hwpx_engine == _SP_INTEL:
        return hwpx_engine, "hwpx_engine"

    is_hwpx, why = _sp_is_hwpx_container(file_path)
    if is_hwpx:
        return _SP_HWPX, why
    if hwpx_engine == _SP_ENGINE_NATIVE:
        # 네이티브를 강제했으면 넘기지 않는다 — 확장자와 내용이 어긋났다는 사실이
        # 폴백에 묻히면 안 된다는 뜻으로 고른 값이다.
        return _SP_HWPX, why
    # 이름만 hwpx 인 파일은 지능형으로 — 실제로 pdf·docx 면 그쪽이 제대로 읽는다.
    return _SP_INTEL, why


def _sp_align_records(records: list) -> list:
    defaults = _sp_schema_defaults()
    for record in records:
...
def _sp_forward_kwargs(kwargs: dict) -> dict:
    return {key: value for key, value in kwargs.items() if key not in _SP_ROUTER_KWARGS}


def _sp_resolve_config_path(explicit):
    """지능형 설정 yaml 을 찾는다. 못 찾으면 `None`(벤더 기본 해석에 맡긴다).

    **지능형 `_load_config` 는 파일이 없으면 예외를 던진다** (첨부용은 `{}` 로 넘어간다).
    즉 여기서 못 찾으면 **처리기 생성 자체가 실패**하고, 그 실패는 `_acquire` 가
    `SmartPreprocessorError` 로 감싸 어느 파일이 없는지 드러낸다 — 조용히 기본값으로
    떨어지는 첨부용보다 오히려 다루기 쉬운 실패다.

    벤더 resolver 는 `Path(__file__)/../resource/…` 를 보는데 **우리 합친 파일은 벤더
    파일과 다른 자리에 놓일 수 있다.** 등록 파일이 어디에 놓이는지 실물로 확인하지
    못했으므로 후보를 넷 둔다.
    """
    for candidate in (explicit, os.environ.get(_SP_CONFIG_ENV, "")):
        ...
        _log_warning("configured vendor config path does not exist - trying the next candidate",
                     event="smart_preprocess_config_missing")
    vendor_resolver = globals().get("_resolve_default_intelligent_config_path")
    ...
    for _ in range(5):
        for sub in ("resource_dev", "resource"):
            for basename in _SP_CONFIG_BASENAMES:
...
class DocumentProcessor:
    ...
    **이 이름이 지능형 절반의 것과 겹쳐서** 그쪽을 `IntelligentDocumentProcessor` 로
    바꿨다(파일 맨 위 표). 안 바꾸면 지능형 것이 이 클래스를 덮어 **hwpx 경로가 영영
    안 탄다** — 오류가 아니라 "표가 깨진 결과" 로만 드러난다.
    """
    ...
    def __init__(self, config_path: str | None = None) -> None:
        ...
        # 지능형은 **그 엔진으로 갈 파일이 처음 들어올 때** 만든다. 생성자가 yaml 을
        # 읽고 docling 변환기·OCR·TableFormer 모델을 올리기 때문에, 여기서 만들면
        # hwpx 만 넣는 배포도 그 비용과 실패 가능성을 함께 진다.
        self._vendor = None
        self._vendor_lock = None
        failure = _sp_engine_error(_SP_INTEL)
        if failure is not None:
            _log_warning(
                "intelligent preprocessing path unavailable",
                event="smart_preprocess_engine_unavailable",
                error_type=type(failure).__name__,
            )

    async def __call__(self, request, file_path: str, **kwargs) -> list:
        started = time.monotonic()
        hwpx_engine = _sp_choice_kwarg(
            kwargs.get("hwpx_engine"), _SP_ENGINE_AUTO, _SP_HWPX_ENGINES, "hwpx_engine"
        )
        overrides = _sp_overrides_kwarg(kwargs.get("route_overrides"))
        engine, reason = _sp_route(file_path, hwpx_engine, overrides)
        _log_info(f"routed to {engine} ({reason})", event="smart_preprocess_routed", status=engine)

        align = _sp_bool_kwarg(kwargs.get("align_vector_schema"), True, "align_vector_schema")

        if engine != _SP_HWPX:
            return await self._run_vendor(request, file_path, **kwargs)

        try:
            records = await self._hwpx(request, file_path, **_sp_forward_kwargs(kwargs))
        except Exception as exc:  # noqa: BLE001 - 분류는 아래 두 줄이 한다
            if hwpx_engine == _SP_ENGINE_NATIVE or _sp_engine_error(_SP_INTEL) is not None:
                raise
            # 적재가 통째로 실패하는 것보다 표가 덜 정확한 적재가 낫다는 판단이다.
            # **다만 조용히 넘기지 않는다**: 이 로그가 없으면 사용자는 표 병합이
            # 보존됐다고 믿는다. 지능형은 hwpx 를 PDF 로 바꿔 읽으므로 표 병합·조문
            # 위계가 남지 않는다.
            _log_warning(
                "hwpx native path failed - falling back to the intelligent path "
                "(표 병합/조문 위계는 보존되지 않는다)",
                event="smart_preprocess_fallback",
                error_code="05-00020003",
                error_type=type(exc).__name__,
            )
            return await self._run_vendor(request, file_path, **kwargs)
        ...
        if align:
            records = _sp_align_records(records)
        _log_info(
            "hwpx native path done",
            event="smart_preprocess_done",
            status=_SP_HWPX,
            item_count=len(records),
    ...
    async def _run_vendor(self, request, file_path: str, **kwargs) -> list:
        processor = await self._acquire(kwargs.get(_SP_CONFIG_KWARG))
        return await processor(request, file_path, **_sp_forward_kwargs(kwargs))

    async def _acquire(self, config_path_kwarg=None):
        ...
        if failure is not None:
            ...
            raise SmartPreprocessorError(
                "지능형 전처리 경로를 사용할 수 없어 이 형식은 처리할 수 없습니다"
        ...
        async with self._vendor_lock:
            if self._vendor is None:
                started = time.monotonic()
                # 생성이 blocking 이다(yaml·토크나이저·docling 변환기·OCR 모델).
                # 이벤트 루프에서 그대로 돌리면 같은 워커의 다른 요청이 그만큼 멈춘다.
                self._vendor = await asyncio.to_thread(self._build, config_path_kwarg)
                _log_info(
                    "intelligent processor initialised",
                    event="smart_preprocess_engine_ready",
                    status=_SP_INTEL,
                    duration_ms=int((time.monotonic() - started) * 1000),
                )
        return self._vendor

    def _build(self, config_path_kwarg):
        resolved = _sp_resolve_config_path(config_path_kwarg or self._config_path)
        try:
            return IntelligentDocumentProcessor(resolved) if resolved else IntelligentDocumentProcessor()
        except Exception as exc:  # noqa: BLE001
            # 설정 파일 부재는 **재시도로 풀리지 않는 배포 문제**다. 원래 예외
            # (FileNotFoundError 등)만 올리면 어느 파일이 없다는 건지 드러나지 않아
            # 몇 번을 다시 눌러도 같은 자리에서 실패한다.
            raise SmartPreprocessorError(
                f"지능형 전처리기를 초기화하지 못했습니다({type(exc).__name__}): {exc}"
            ) from exc
