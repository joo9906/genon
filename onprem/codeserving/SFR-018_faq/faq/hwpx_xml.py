"""hwpx XML 이름과 문단 소유 판정 — 읽기(`hwpx_text`)와 쓰기(`exporters/hwpx_export`)가 공유한다.
...
읽기 경로에서는 마크다운이 통째로 깨지고, 쓰기 경로에서는 값이 엉뚱한 셀에 들어간다.

같은 규칙을 두 벌로 들고 있으면 한쪽만 고쳐 두 경로의 판정이 갈린다. 판정은 여기
한 곳에 두고, 그 위에 각자의 용도를 올린다 — 읽기는 텍스트를, 쓰기는 노드를 쓴다.
