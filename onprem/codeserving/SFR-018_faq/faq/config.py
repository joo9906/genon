...
class Config:
    ...
    # ── 생성 개수 (요구사항 §4) ──
    #
    # **사용자가 고르는 개수는 "문서 한 구간에서 뽑을 개수" 다** (2026-08-31 의미 변경).
    # 그전에는 전체 개수였고, 그러면 긴 문서에서 **같은 개수를 구간 수로 나눠 갖는다** —
    # 40구간 문서에서 5개를 요청하면 구간당 0~1개라 각 구간을 대표하지도 못하고,
    # 문서가 길어질수록 한 구간의 몫이 0 에 가까워진다(그 구간 내용은 후보에서 빠진다).
    # 지금은 구간마다 이 개수를 뽑고 **전체는 아래 총량 상한**이 잡는다.
    #
    ...
    # 전체 총량 상한 — 구간당 개수 × 구간 수가 여기서 멈춘다. 기본값 30 은 구간당 5개
    # 기준으로 **여섯 구간**이다(요구 확정, 2026-08-31).
    #
    # 이 값이 없으면 문서 길이가 곧 항목 수이고 곧 LLM 비용이다. 상한에 걸려 몫을 받지
    # 못한 구간은 **문서 앞뒤로 치우치지 않게 고르게 건너뛰고**(`chunking.plan_quota`),
    # 그 사실을 `coverage_capped` 로 낸다 — 조용히 건너뛰면 사용자는 문서 전체에서
    # 뽑은 결과로 읽는다.
    MAX_TOTAL_FAQ_COUNT = int(os.environ.get("FAQ_MAX_TOTAL_COUNT", "30"))
    ...
    #
    # **호출 수는 이 값이 아니라 총량 상한이 정한다** — 조각이 40개여도 구간당 5개 ·
    # 총량 30개면 LLM 은 6번 부른다(`chunking.plan_quota`).
