"""jinja 프롬프트 로더 — 문구는 `onprem/prompt/SFR-018_text_polish/*.j2` 에 있다.

번역·FAQ·006 단위의 `prompt_loader.py` 와 같은 계약의 사본이다 (배포 단위 간 import 금지).
설계 근거는 그쪽 머리말과 같다:
- 문구 수정이 코드 리뷰·재빌드 없이 끝나고, GenOS Prompt 리소스(10.5절)로 옮기기 쉽다.
- 템플릿이 없으면 **빈 프롬프트로 넘어가지 않고 실패한다.** 지시문 없는 프롬프트로
  LLM 을 돌리면 원문을 그대로 되뱉거나 제멋대로 고친 결과가 정상 응답처럼 내려간다.
- `StrictUndefined` — 변수 오타를 빈칸으로 렌더하면 톤 지시 한 줄이 조용히 사라진다.

배포 전제: 프롬프트 디렉토리는 배포 단위 **바깥**이므로 이미지에 함께 넣어야 한다.
위치가 다르면 `POLISH_PROMPT_DIR` 로 통째 지정한다. 이 단위는 워크플로우(02) 전용이라
**워크플로우 이미지에 `jinja2` 가 필요하다** (가이드 §5.5 — 워크플로우 단계는 임의
패키지 추가가 불가하므로 이미지에 미리 들어가 있어야 한다).
"""
