...
| `check_unit_endpoints.py` | 51 | 특성화 | **018 세 단위**(번역·글다듬이·FAQ) 엔드포인트 경계 + **txt 규약 3단위 대조** + 설정 부재가 내부·실행 실패와 갈리는가 — 위 점검이 006 전용이라 생긴 구멍을 메운다 |
...
| `check_mcp_tools.py` | 40 | 실행 | **MCP 도구 파일 4개** — 한 서버에 올려도 안 덮이는가, 결정적 판정이 나오는가, 빈 문자열 주입을 견디는가 |
...
## 상태 (2026-08-14 — **11개 스크립트 전부 종료 코드 0**, OK 366건 / WARN 3)
