...
| `check_mcp_tools.py` | 86 | 실행 | **MCP 도구 파일 4개** (2026-09-07 에 hwpx 파싱 도구가 빠져 5 → 4) — 한 서버에 올려도 안 덮이는가, 결정적 판정이 나오는가, 빈 문자열 주입을 견디는가 + **선택지가 도구 스키마(enum)에 실리는가** + **원문 언어 교차검증** + **관리자 정책 반영** + **`diff_changes` 낱말 좌표·`<mark>` 사본·보호 구간**(HTML 표 셀·코드펜스·삭제·상한) |
...
| `check_table_grid.py` | 34 | 사본 대조 | 006↔번역↔FAQ **파싱 코어** 일치 (전처리기 2벌까지 **5벌**. MCP 사본은 2026-09-07 에 빠졌다) (텍스트가 아니라 출력으로 대조). 3층 "누락 방지"(상자·자동 번호·tail·수식)는 **전처리기를 정본으로** 함께 본다 |
