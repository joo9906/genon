...
- **첫 턴에만 태운다** (요구 확정). 판정이 둘이다 — 값·블록이 이미 있으면(대화가 시작됐다)
  안 돌고, **세션의 `source_doc_hash` 가 같으면** 안 돈다. 뒤엣것이 없으면 캔버스가 매 턴
  같은 `genosUploaded` 를 실어 올 때 문서를 다시 태워 **사용자가 지운 값을 우리가
  되살린다**(오류가 나지 않아 제보로만 드러난다). 표식은 저장이 덮어쓰기라 **커밋이 매 턴
  다시 실어야 한다.** 값을 하나도 못 뽑은 문서에서는 **표식이 유일한 방어선**이다(값이
  0개라 "이미 시작됐다" 판정이 걸리지 않는다).
...
cd SFR-006 && python -m unittest discover -s tests -t .   # 48건 (**문서 자동 채움** `test_doc_prefill` 16건 포함)
...
python onprem/test/check_workflow_run.py    # 85건 — 워크플로우 스텝 9개 실행·반환형·result 1회
...
python onprem/test/check_final_preprocessor.py  # 119건 — **합친 등록 단위**(area 05, 벤더 2 + hwpx)
                                            ...
                                            #    경로가 어긋나면 건수가 조용히 113 으로 줄어든다.
                                            #    파일을 옮기면 `_SAMPLES` 를 같이 고칠 것.
                                            # ※ docling 이 **반쯤 설치된** 환경에서는 `벤더 부재:
                                            #    어느 엔진인지…` 1건이 FAIL 한다 — 판정이
                                            #    `ModuleNotFoundError` 문자열을 보는데 그 환경은
                                            #    `ImportError` 를 낸다. 코드 결함이 아니다.
...
python onprem/test/check_unit_endpoints.py  # 75건 — 018 세 단위 엔드포인트 경계
...
python onprem/test/check_chat_turn.py       # 34건 — 대화 한 턴 계약·상태 전이 + 설정 부재 분류
                                            #        + **업로드 문서 자동 채움**(첫 턴만·발화 우선·
