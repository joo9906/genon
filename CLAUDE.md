...
- **준비된 것**: 호출부가 논리 이름만 넘기므로 소스 교체는 `prompt_loader.render`
  안에서 끝난다 (위 컨벤션).
- **남은 불일치 세 가지** (seam 을 고쳐도 안 없어진다):
  1. `render()` 는 동기, `load_prompt` 는 async HTTP. 동기 호출부
     (`prompts.build_extract_prompts` 등)가 있어 async 화 또는 기동 시 1회 캐시가 필요하다.
  2. **jinja 제어문을 GenOS `PromptTemplate` 가 못 받는다.** `{question}` 류 단일 치환만
     된다 — 번역 용어사전 `{% for %}`, FAQ 재요청 질문 목록, 글다듬이
     `{% if doc_type_instruction %}` 는 코드가 문자열로 조립해 변수 하나로 넘겨야 한다.
  3. §10.5 가 프롬프트 ID 하드코딩을 금지한다 → 환경변수가 j2 파일 수만큼(현재 12개) 생긴다.
- **워크플로우(02)는 HTTP 직접 호출이 아니라 Flowise `Prompts > MNC Prompt` 노드**를
  쓰라고 돼 있다. 우리 02 단위 셋은 Python 단계라 노드를 못 쓴다 — 실제 캔버스 구성을
  봐야 판단이 된다(플랫폼 팀 확인 사항).
