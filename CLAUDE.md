...
- **실물 네 벌의 출력은 변경 전후가 바이트까지 같다** — 되살린 경로가 기존 문서를
  건드리지 않는다는 뜻이다.
- **파싱 코어 사본 넷(MCP·번역·FAQ·006)에는 이 층이 없다.** 저쪽은 **요청 경로**라
  사용자가 결과를 눈으로 보고, 전처리기는 **적재 경로**라 아무도 못 본다 — 지금은 일부러
  다르다. 옮길 때는 표 격자처럼 대조 점검을 함께 세울 것.
...
cd SFR-018 && python -m unittest discover -s tests -t .   # 195건 (표 HTML 전환·preprocessor 조문 위계·**전처리기 누락 방지(상자·자동 번호·tail)**·표 조각 머리말·초과 행 분할·표 조각 번호 규약·용어사전 적용 범위·<strong> 사본 조립 포함)
...
**11개 + unittest 2벌. 위 건수는 2026-08-19 에 전부 돌려서 확인한 값이다**
(2026-08-19 에 움직인 것은 SFR-018 unittest 172→**195** 하나다 — 전처리기 누락 방지).
