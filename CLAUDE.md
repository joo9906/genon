...
### PII 마스킹 누락을 **네 기능 출구에서** 잡는다 — eval (2026-09-02)

민감정보 마스킹(#315)은 적재 전처리기(첨부용 `guardrail`)의 몫인데 **그 층이 없어도
조용하다.** 사이트 설치본에 모듈이 없거나(2026-09-02 에 실제로 그 import 가 없어 pdf
적재가 막혔다) 설정이 꺼져 있으면 적재는 성공하고 검색도 되며, 네 기능이 그 텍스트를
최종 답변에 **원문 그대로** 싣는다. 오류가 없으니 로그에도 안 남는다 — 사용자가 화면에서
주민등록번호를 발견할 때까지 아무도 모른다. `pii_leak_count` 가 그 출구를 본다.

- **비율이 아니라 절대 건수다.** 허용치가 0인 값에 비율 기준을 걸면 200문장 문서의
  1건이 0.5% 라 임계를 통과한다 — **기준이 사고를 승인한다.**
- **네 기능이 같은 지표를 같은 방식으로** 돈다(`suites._run_pii`). 마스킹 누락은 기능
  고유의 실패가 아니라 그 텍스트가 넷을 모두 통과하는 것이라, 기능마다 다르게 재면 안 된다.
  **FAQ 는 이것이 유일한 운영 기준이 됐다**(근거성에는 기준을 두지 않는다).
- **`leak_count == 0` 은 "개인정보 없음" 이 아니다.** `detectors` 에 적힌 유형만 본다 —
  이름·주소·계좌번호는 안 센다(NER 이 없고, 계좌번호는 체크섬이 없어 일반 숫자열과
  구분되지 않는다). 그 공백을 응답에 싣는다. `masked_count` 도 함께 낸다 —
  `leak=0 · masked=0` 은 "개인정보가 없던 문서" 와 "아무것도 안 돌았다" 가 구분되지 않는다.
- **체크섬으로 오탐을 거른다**(주민·사업자·카드). **오탐은 결국 미탐으로 간다** —
  가드레일이 상시 빨간불이면 사람이 임계를 올리거나 지표를 끈다.
- **검출한 값은 응답에 담지 않는다**(3.8절). 자리와 유형만 낸다 — 담으면 평가 리포트가
  유출 경로가 된다.

**같이 고친 것 — `<mark>` 가 채점을 무너뜨리고 있었다.** payload 에 남은 텍스트는
하이라이트 사본이고 정본은 파일로만 있다(2026-08-28). 태그째 채점하면 어미가
`…습니다</mark>` 로 끝나 종결어미 판정이 전부 `other` 로 떨어지고 `ending_consistency`
가 **불합격이 아니라 미측정으로 조용히 빠진다.** `pairs.pair_texts` 한 곳에서 벗긴다
(다섯 집계기가 전부 거기를 지난다). **`<mark>` 만** 벗기고 `<table>` 은 남긴다.

**그물**: `check_eval_metrics` 68 → **80건**. 되돌려 FAIL 을 본 갈래는 셋이다 —
`pair_texts` 의 태그 제거(3건 동시 FAIL) · 체크섬 제거(오탐) · `scan_text` 의 태그
제거(미탐). 완전 입력 픽스처에 `answers` 를 넣지 않으면 **네 기능의 기준 도달 판정이
전부 FAIL** 하는 것도 확인했다(그 층이 이번에도 제일 먼저 잡았다).
...
- **FAQ 는 조각마다 자기 몫을 만든다.** 호출 수는 **총량 상한**이 정한다 — 구간당
  5개 · 총량 30개면 조각이 40개여도 6번 부르고, 태울 조각을 **고르게 표집한다**
  (`plan_quota`). 앞에서부터 채우면 잘라 쓰던 시절과 결과가 같아진다.
  - **사용자는 총 개수만 고르고 배분은 우리가 한다 (2026-09-03 요구 확정).** 고른
    숫자가 곧 받는 개수다. 2026-08-31~09-02 에는 선택이 **구간당** 개수였고 총량을
    `FAQ_MAX_TOTAL_COUNT` 가 잡았는데, 그러면 **고른 숫자와 받는 개수가 달라진다**
    (구간이 여섯이면 5를 골라도 30개). 그 규약이 막으려던 것(긴 문서에서 조각당 몫이
    0 에 가까워져 그 구간 내용이 후보에서 빠지는 것)은 이제 **호출 수 상한**
    (`FAQ_MAX_CHUNK_CALLS`, 기본 6)이 막는다 — 태울 구간 수가 묶여 있어 조각이 40개여도
    여섯 구간이 나눠 갖고 몫은 1 밑으로 내려가지 않는다. **개수는 사용자가, 비용은
    배포가** 정한다: 한 손잡이에 묶으면 둘 중 하나를 못 지킨다.
    옛 `FAQ_MAX_TOTAL_COUNT` 는 없앴다(언제나 안 걸리는 상한은 읽는 쪽이 "확인했다" 고
    믿게 만든다). `FAQ_MAX_COUNT` 기본값은 10 → **30** 이다 — 옛 총량 상한과 같은 값이라
    실제로 만들 수 있던 최대치가 유지된다.
  - **상한에 걸려 못 태운 구간은 `coverage_capped` 로 알린다.** `source_truncated`
    (조각 수 상한)와 **다른 사건**이라 갈라 뒀다 — 그쪽은 문서 뒤를 아예 안 본 것이고
    이쪽은 전체를 나눴지만 일부만 태운 것이다. 조용히 건너뛰면 사용자는 문서 전체에서
    뽑은 결과로 읽고, **안 나온 내용이 문서에 없는 것으로 보인다.**
  - **`requested_count` 는 사용자 선택 그대로다** (2026-09-03 되돌림). 조각 수와 무관
    하고 상한으로 깎인 값만 반영한다 — 2026-08-31~09-02 에는 "배정된 몫의 합" 이었고
    그때는 안내문이 "5개 중 28개를 만들었습니다" 가 되지 않게 하려는 보정이었다.
    선택이 총 개수가 된 지금은 그 보정 자체가 필요 없다. `per_chunk_count`·`total_cap`
    필드는 없어지고 **호출 수 상한이 `call_cap`** 으로 나간다.
  - **번역·글다듬이에는 이 배분이 없다** — 그 둘은 개수를 뽑는 것이 아니라 **모든
    유닛·조각을 처리**하므로 나눌 몫 자체가 없다. 같은 방식을 얹을 자리가 아니다.
  - **조각 호출은 2026-09-09 부터 병렬이다** (아래 "FAQ 조각을 병렬로 부른다" 절).
...
- **번역은 이 배분과 무관하다 — 모든 유닛을 번역한다.** 다만 같은 성격의 **조용한
  절단이 한 자리 남아 있었고 2026-08-31 에 고쳤다**: `POST /translate/hwpx` 만 상한을
  파서에 넘겨(`to_markdown(max_chars=500,000)`) 넘는 만큼을 **버렸다.** 번역 쪽
  `HwpxDocument` 에는 `truncated` 필드가 없어 **응답에도 로그에도 흔적이 없고**,
  사용자는 뒷부분이 빠진 번역문을 받는다. 나머지 세 경로는 초과를 오류로 세우고 있었으니
  **같은 문서를 어느 경로로 넣었는지에 따라 결과가 달랐다.** 이제 넷이 같다.
...
순서가 아니다. 흘리려면 **LLM 이 마크다운 본문을 그대로 내야** 하는데, 그러면 스켈레톤
분해가 성립하지 않는다 — **구조를 지키는 주체가 코드에서 프롬프트로 넘어간다.**
요구사항 §5 의 "표 깨짐" 을 코드로 막던 장치를 내려놓는 것이라 정본에 얹지 않았다.

- **감수한 것을 감추지 않는다.** 우리가 깨뜨리지는 않고(조각을 코드펜스·표 가운데에서
  끊지 않는다), 끝나고 **지문을 대조해 알린다**(`stream_pipeline.structure_diff` —
  표 행·셀·펜스·제목 개수). 글다듬이 `markdown_guard` 와 같은 취지다. 정본 경로
  (`POST /translate/markdown`)는 그대로 있고 **표가 많은 문서는 그쪽이 맞다.**
- **두 번 부른다** — 흘리고(`POST /translate/stream`, SSE), 끝나면
  `POST /translate/finalize` 가 `{original_text, translated_text, glossary, structure,
  download_url}` 을 준다. 하이라이트가 번역이 끝나야 정해지기 때문이다: 준수 판정은
  번역문 전체를 봐야 하고 좌표는 최종 문자열 기준이라 흐르는 도중에는 없다.
- **흘리는 것은 정본이다.** 사본(`<mark>`)을 흘리면 ① 하이라이트가 스트리밍 중에 먼저
  나타나 요구가 말한 순서와 어긋나고 ② 태그가 조각 경계에서 갈려 부스러기가 남는다.
- **finalize 는 무상태다.** 프론트가 방금 받은 두 텍스트를 되돌려 보내면 서빙이 대조를
  다시 한다 — 용어 매칭은 결정적이라 같은 답이 나오고, 세션을 붙이면 이 무상태 단위에
  상태가 생긴다. LLM 을 부르지 않으므로 재시도해도 안전하다. **`done` 이 준
  `translated_text` 를 그대로 보내야 한다** — 프론트가 델타를 이어 붙인 값과 한 글자라도
  다르면 좌표가 밀리고, 그 어긋남은 **하이라이트가 한 칸 밀린 화면**으로만 드러난다.
- **순서 버퍼·전량 실패 규약은 글다듬이와 같은 코드다** (2026-09-09 절). 머리 조각은
  라이브로, 뒤 조각은 버퍼에 모았다가 차례에 한 번에. **전량 실패에서는 한 글자도
  흘리지 않는다** — 실패 자리의 원문을 즉시 흘리면 오류로 갈아엎을 때 답이 나왔다가
  사라진다.
...
- **미검증**: 실환경에서 SDK 경로가 뜨는지, 게이트웨이가 `stream=True` 를 받는지,
  중간 프록시가 SSE 를 모아 보내지 않는지, `model` 값을 게이트웨이가 어떻게 다루는지.

### 반입 판본을 **`not/` 하나로** 합쳤다 (2026-09-09 요구 변경)

사내 mirror 에 **`lxml`·`openai` 가 들어왔다.** 그래서 판본 셋 중 둘의 존재 이유가
없어졌다 — `not/` 은 "lxml 이 없어 깎아낸 것" 이고 `not/openai/` 는 "SDK 도 되는지 보려고
나란히 둔 것" 이었다. **반입 대상은 `not/` 하나이고, 기능이 가장 많은 판본이 됐다.**
...
| `not/openai/` | 별개 판본(62건 그물) | **흡수** → `archive/not_openai_absorbed/` |

- **깎아낸 코드를 되살려 붙이지 않았다.** `not/openai/` 가 이미 "정본 + SDK + lxml +
  번역 스트리밍" 이었으므로 **네 단위를 그것으로 교체**했다. 되살려 붙이는 쪽은 정본과
  갈릴 자리를 새로 만든다.
- **`not/openai/` 를 지우지 않고 옮겼다.** `not/` 전체가 **git 미추적**이라 지우면
  복구할 수 없다. 반입 표면에서 near-duplicate 만 없앤 것이다.
- **FAQ 스트리밍은 엔진만 있고 HTTP 로 안 열려 있었다.** 증분 파서(`markdown_items.py`)·
  `faq_stream_async`·`generate_faqs_stream` 이 다 있는데 `main.py` 가 import 조차 하지
  않아 **호출부 0건**이었고, 그 판본의 점검은 오히려 **"FAQ 는 스트리밍이 없다" 를
  기대**해 FAIL 1건으로 남아 있었다(점검 14:38, 기능 14:51~14:55 — **그물이 기능보다
  먼저 굳은** 자리다). `POST /generate/stream` 을 붙였다.
- **FAQ 는 토큰을 그대로 흘리지 않는다.** 항목이 스키마·근거·중복을 지나야 화면에 나갈
  자격이 생기므로, 프롬프트가 `근거 → 질문 → 답변` 순으로 쓰게 해서 검증을 **접두어
  연산**으로 만든다 — 근거가 닫히면 대조하고(실패면 한 글자도 안 나간다), 질문이 닫히면
  중복을 보고, **답변 첫 델타에서 항목을 연다.** 질문이 닫힐 때 열지 않는 이유는 답변이
  끝내 안 오는 경우(형식 위반)에 **답 없는 질문이 화면에 남기** 때문이다.
- **payload 조립은 한 함수다**(`_store_and_payload`). 스트리밍·비스트리밍이 각자
  조립하면 `markdown`·`download_url`·`download_ready`·세션 저장 넷 중 하나가 한쪽에만
  붙고, 그 어긋남은 오류가 아니라 **다운로드 버튼이 한 경로에서만 켜지는** 형태로만
  드러난다.
...
- **그물의 요점은 "갈리는 자리 목록" 이다.** 당시 `EXPECTED_DIFF`(17) ·
  ...
  **정본에만 있는 파일 0**. 마지막 것이 `lxml` 로 되살린 파일 넷(006 `hwpx_style.py` ·
  ...
  정본 변경이 흘러든 것이고, 목록에 있는데 같아지면 이 판본의 기능이 되돌려진 것이다.
- **여섯 갈래를 되돌려 FAIL 을 확인했다**: FAQ 스트리밍 라우트 제거 · 목록 밖 파일
  드리프트 · hwpx 파서 파일 누락(**기동·라우트 판정까지 함께** FAIL) · 근거 기각 항목
  흘려보내기 · `lxml` 선언 제거 · 중복 판정 제거.
- **미검증**: SDK 경로가 실환경에서 뜨는지(정본이 2026-09-07 에 SDK 를 걷어낸 이유가
  "실환경에서 SDK 때문에 호출이 실패했다" 였고 그 원인은 **가려지지 않았다** — 되돌릴
  자리는 네 `llm.py` 뿐이다) · 게이트웨이가 `stream=True` 를 받는지 · 중간 프록시가
  SSE 를 모아 보내지 않는지 · `model` 값을 게이트웨이가 어떻게 다루는지.

### 스트리밍이 **캔버스에 닿지 않고 있었다** (2026-09-09)

서빙 세 경로를 다 만들었는데 **워크플로우 스텝이 비스트리밍을 부르고 있었다.** 즉
`not/openai/` 의 FAQ 스트리밍이 "엔진만 있고 `main.py` 가 import 조차 안 해 호출부
0건" 이던 것과 **같은 형태가 한 층 위(서빙 → 스텝)에서 반복**된 것이다.

| 스텝 | 부르던 곳 | 화면 |
|---|---|---|
| 글다듬이 | `/polish/stream` | **진짜로 흐른다** |
| 번역 | `/translate/markdown` | 완성 뒤 `_stream_chunks` 연출 |
| FAQ | `/generate` | 완성 뒤 `_stream_chunks` 연출 |

`emit` 뒤 `sleep(0)` 은 지연이 아니라 스케줄러 양보라 조각이 수 ms 안에 다 나가고
브라우저는 한 프레임에 그린다 — **"한방에" 와 구분되지 않는다.**

- **스텝 사본을 만들지 않았다.** 정본 스텝에 **SSE 시도 + 폴백**을 넣었다(글다듬이가
  이미 그 모양이다). 정본 서빙에는 스트리밍 라우트가 없으므로 SSE 아닌 응답이 와서
  **자동으로 폴백**하고 지금 동작 그대로다 — 흐르는 것은 `not/` 서빙뿐이다. 스텝을
  `not/` 에 두면 반입 표면이 늘고 사본 드리프트가 새로 생긴다.
- **SSE 리더는 `_stream_serving(env_name, path, payload, *, read_timeout)` 하나다** —
  `_post_serving` 과 시그니처를 맞췄고 **세 스텝이 같은 이름·같은 코드**로 든다.
  `check_deploy_contract` 의 "워크플로우 스텝 사본 일치" 가 **같은 이름의 함수는 같은
  본문**이라야 통과시키므로, 이름을 단위마다 다르게 두면 그 대조를 빠져나간다.
- **프레임 종류를 리더가 열거하지 않는다.** `done`·`error` 가 아니면서 `text` 를 든
  프레임은 무엇이든 흘린다 — 글다듬이·번역은 `delta` 하나지만 FAQ 는 `item_open`·
  `item_close` 도 화면 조각을 들고 온다. 종류를 리더에 적으면 단위가 프레임을 하나
  더할 때 **세 스텝을 모두** 고쳐야 하고, 안 고치면 그 조각이 조용히 화면에서 빠진다.
- **번역은 두 번 부른다** — 흘리고(`/translate/stream`), 끝나면 `/translate/finalize`.
  `_finalize_stream` 이 그 둘을 **비스트리밍 응답 모양으로 정규화**한다. 뒤 코드가 두
  모양을 각각 분기하면 조립이 두 벌이 되고 한쪽만 고치는 실수가 화면에서만 드러난다.
  **finalize 실패는 fail-open** 이다 — 번역문은 이미 흘렀으므로 되돌릴 수 없고,
  하이라이트가 없다고 결과를 버리면 **사용자가 방금 본 글이 오류로 바뀐다.**
  - **그래서 `finalize` 가 사본도 낸다** (`markdown_highlighted`·
    `source_markdown_highlighted`, **`/translate/markdown` 과 같은 키 이름**). 좌표만
    주면 태그를 끼우는 쪽이 스텝이 되고 겹침 병합·역순 삽입 규칙이 워크플로우에 한 벌
    더 생긴다. 조립은 `highlight_units` 를 그대로 쓴다.
- **FAQ 는 화면 조각을 서빙이 프레임에 실어 준다**(`_display_text`). 스텝이 제목 줄을
  조립하면 FAQ 화면 형식이 워크플로우에도 한 벌 생긴다. 형식이 서빙 안에서 두 곳
  (`_display_text` · `formatting._render`)인 것은 남지만, 갈리지 않는 근거를 **등식**
  으로 뒀다 — 흘린 조각을 이어 붙이면 최종 마크다운과 같아야 한다.
- **흘리는 시점이 전량 폴백 판정보다 앞이다** (번역). 비스트리밍은 판정을 끝내고
  흘렸는데 여기서는 흘리면서 간다. 성립하는 근거는 **서빙 쪽 보장**이다 — 전량 실패에서
  한 글자도 흘리지 않는다. 그 보장이 깨지면 답이 나왔다가 사라지는 화면이 된다.
- **그물이 이 층을 하나도 안 보고 있었다.** `_stub_gateway` 는 `_post_serving` 만
  바꾸므로 그대로 두면 `_stream_serving` 이 실패해 **폴백으로 지나가고 스트리밍 경로를
  한 줄도 태우지 않는다.** `check_workflow_run` 108 → **118**(번역 5 + FAQ 5),
  `not/check_not_units` 91 → **92**(서빙 등식). **네 갈래를 되돌려 FAIL 을 확인했다** —
  조건부 스트리밍 제거(번역·FAQ **각각**, 두 번 흘린다) · `finalize` 호출 제거 ·
  리더를 `delta` 만 흘리게 되돌리기. 등식 판정은 `**Q{n}.` 의 번호를 하나 밀어 확인했다
  (**길이가 같은 어긋남**도 잡는다).
- **`_NO_STREAM_STEPS` 가 FAQ 를 아직 붙들고 있었다.** FAQ 스트리밍은 2026-09-02 에
  되살아났는데 그물이 따라오지 않아 **그때까지 한 번도 검사된 적이 없었다**(CLAUDE.md
  가 미해결로 적어 두었던 그 자리다). 네 스텝이 다 흘리므로 그 목록은 비었고, FAQ 는
  "오류 경로에서는 흘리지 않는다"(`_NO_STREAM_ON_ERROR`) 쪽으로 옮겼다.
- **미검증은 그대로다** — 게이트웨이가 `stream=True` 를 받는지, 중간 프록시가 SSE 를
  모아 보내지 않는지.

### FAQ 조각을 **병렬로 부른다** (2026-09-09)

긴 문서를 조각으로 나누는 것은 2026-08-29 에 붙였는데, **그 조각들을 혼자 순차로 돌고
있었다** — 번역은 배치를 `LLM_CONCURRENCY`(15)로, 글다듬이는 조각을
`POLISH_LLM_CONCURRENCY`(4)로 겹쳐 도는데 FAQ 만 `for` 루프 안에서 하나씩 `await`
했다. **조각 사이에 순서 의존이 없다** — 앞 조각의 결과가 뒤 조각의 프롬프트에 들어가지
않으므로 순차로 돌 이유가 없었고, 대기시간만 조각 수에 그대로 비례했다(기본 호출 상한
6조각이면 한 번 호출 시간의 여섯 배다).

- **호출만 겹치고 채택은 조각 순서대로 한다.** `_request_chunk` 는 공유 상태를 건드리지
  않는 순수 호출이고(프롬프트 렌더 + LLM), 판정·채택은 `generate_faqs` 가 `gather` 결과를
  받아 **조각 순서대로** 흘린다. 도착 순서대로 채택하면 중복 판정(`seen_questions`)·
  기각 건수·**조각별 채택 상한**(`현재 + 몫`)이 전부 누적 상태라 **같은 문서가 실행마다
  다른 분포를 낸다** — 오류로는 드러나지 않고 "어느 구간에서 몇 개가 나왔나" 만 흔들린다.
  즉 병렬로 바뀐 것은 **타이밍뿐이고 결과는 순차 시절과 같다.**
- **첫 실패에서 멈추지 않는다** (멈출 것이 없다). 순차 시절에는 실패한 조각에서 `break`
  했지만 병렬에서는 호출이 이미 전부 나갔다 — 성공한 조각의 결과를 버리는 것은 손해다.
  실패는 **조각 순서 기준 첫 건**을 기록하고(전량 실패면 그것이 사용자에게 나가는 분류다),
  건진 항목이 있으면 기존 부분 실패 규약대로 `chunks_used < chunks_planned` 로만 남는다.
- **동시 수는 별도 손잡이다** — `FAQ_LLM_CONCURRENCY`(기본 6). `FAQ_MAX_CHUNK_CALLS` 는
  "몇 번 부르나"(비용)이고 이쪽은 "동시에 몇 개가 도나"(대기시간)다. 기본값을 그 상한과
  같게 둬서 **기본 설정에서는 배정된 조각이 한 번에 다 뜬다** — 상한을 올린 배포에서만
  둘이 갈리고, 그때 게이트웨이에 실리는 부하는 이 값이 잡는다. 429 가 나면 여기부터 내린다.
- **조각 크기 기본값을 24,000 → 12,000 으로 내렸다** (요구). 순차 시절에는 잘게 쪼개면
  호출 수만 늘어 손해였지만, 겹쳐 도는 지금은 **조각 하나의 크기가 곧 전체 대기시간**이다.
  덮는 문서 길이가 줄지 않게 `FAQ_MAX_CONTEXT_CHUNKS` 를 40 → 80 으로 함께 올렸다
  (80 × 12,000 ≈ 96만 자로 그대로다) — 안 올리면 `source_truncated` 문턱이 조용히 절반이
  되고, 그 상태는 "왜 뒤쪽에서 FAQ 가 안 나오나" 로만 드러난다.
- **부족분 재요청(`_fill_shortfall`)은 순차로 둔다.** 그쪽은 **이미 채택된 항목 목록을
  프롬프트에 실어** 다시 부르는 호출이라 앞 호출의 결과가 뒤 호출의 입력이다 — 겹쳐
  돌리면 같은 질문을 다시 받는다. 호출도 총 2회 상한이라 얻을 것이 없다.
- **스트리밍은 건드리지 않았다.** FAQ 스텝이 흘리는 것은 서빙이 조립을 끝낸 마크다운이고
  (2026-09-02), 근거·중복 기각을 지나기 전 항목을 흘리지 않는다는 규약이 그대로다.
- **그물**: `test_faq_chunking` 의 `ParallelChunkCallTest` **신규 4건**. 동시 호출 여부는
  **타이밍이 아니라 `peak`(동시에 떠 있던 수)** 로 본다 — 순차 코드에서는 어떤 타이밍에도
  1 을 넘을 수 없어 흔들리지 않는다. **세 갈래를 되돌려 FAIL 을 확인했다**: `gather` 를
  순차 `await` 로(3건 동시 FAIL) · 채택을 도착 순서로(분포가 뒤집힌다) · 동시 수 상한.
  기존 27건(→31)과 `check_unit_endpoints` 113 · `check_workflow_run` 108 은 그대로다.

### 글다듬이는 **다듬어지는 대로** 흘린다 — 연출이 아니라 진짜 스트리밍 (2026-09-09)

아래 2026-09-01 절이 되살린 것은 **완성 뒤의 연출**이었다. 서빙이 문서를 다 다듬어 한 번에
주고, 스텝이 그 글을 조각내 흘린다 — 그러니 사용자가 기다리는 수십 초 동안은 화면이 그대로
비어 있다. 게다가 `emit` 뒤의 `await asyncio.sleep(0)` 은 **지연이 아니라 스케줄러 양보**라
(가이드 5.2 의 write buffer flush) 조각 전부가 수 ms 안에 나가고 브라우저는 한 프레임에
그린다 — **조각을 나눈 것이 화면에서는 "한방에" 와 구분되지 않는다.** 네 스텝이 다 그렇다.

**글다듬이만 고쳤다. 넷 중 이 방식이 성립하는 유일한 단위다:**

| 단위 | LLM 출력 | 진짜 스트리밍이 되나 |
|---|---|---|
| 글다듬이 | **마크다운 본문 그대로** (`system.txt`: "다듬은 글만 반환합니다") | **된다** |
| FAQ | JSON 스키마 | 안 된다 — 원시 `{"question": …` 이 보이고, **근거·중복 기각을 지나기 전** 항목이 흘러 기각될 항목이 나타났다 사라진다 |
| 번역 | 배치 JSON (`{id, t}`, 15개 동시) | 안 된다 — 같은 이유 + 유닛 순서가 배치에 묶여 있다 |
| 006 | 발화 추출 JSON → 안내문 조립 | 안 된다 — 흘릴 것이 LLM 출력이 아니다 |

- **경로가 하나 늘었다** — `POST /polish/stream`(SSE). `POST /polish` 는 **폴백으로 남는다**:
  폐쇄망에서 게이트웨이가 `stream: True` 를 받는지 **실물로 확인되지 않았고**(네 단위가
  지금까지 `"stream": False` 를 명시해 온 이유가 그것이다), 안 받는 배포에서 기능이 통째로
  죽으면 안 된다.
- **폴백은 서빙이 흡수한다.** 게이트웨이가 스트리밍을 거절하면(`STREAM_UNSUPPORTED` —
  400·415·422·501) 서빙이 그 자리에서 `polish_document` 로 되돌아가 결과를 한 덩어리로
  흘린다. **스텝은 SSE 하나만 알면 된다** — 스텝에 두면 캔버스에 등록된 파일을 고쳐야
  폴백이 바뀌고, 그 파일은 사본 대조 대상이라 넷을 함께 고치게 된다. (스텝에도 한 겹 남긴
  폴백은 **서빙 판본이 어긋난 배포**용이다: SSE 가 아닌 응답이 오면 `POST /polish` 로 간다.)
- **순서 버퍼** — 조각은 세마포어 상한 안에서 함께 도므로 조각 3이 조각 1보다 먼저 끝난다.
  끝난 순서대로 흘리면 문단이 뒤섞이는데, **`result` 가 갈아 끼우므로 최종 결과는 멀쩡하다** —
  스트리밍 중에만 틀리고 로그에는 아무것도 안 남는다. 그래서 병렬은 유지하고 흘리는 순서만
  묶었다: **머리 조각**은 델타가 오는 대로 라이브로, **뒤 조각**은 자기 버퍼에 모아 두고
  차례가 오면 **한 번에** 붙인다(요구 확정).
- **흘린 것 == 정본이 계약이다.** `llm.polish_stream_async` 가 델타를 이어 붙인 것을
  그대로 `content` 로 내고 **`strip()` 을 걸지 않는다**(조각 사이 공백은 `rebuild` 가
  `suffix` 로 되꽂는다). 어긋나면 화면이 순간 다른 글을 보여주고 오류로는 안 드러난다.
- **첫 델타 뒤에는 재시도하지 않는다.** 다시 부르면 그 출력이 화면에 **이어붙어** 같은
  문장이 두 번 나온다 — 사용자는 그것을 결과물로 읽는다. 그래서 흘린 뒤의 실패는 그 조각의
  실패로 확정하고, 화면과 정본이 어긋난 사실을 `stream_diverged` → 안내문으로 낸다.
- **실패 조각의 원문은 최종 판정 뒤에만 흘린다.** 실패 자리에 원문을 되꽂는 것은 기존
  규약인데(`chunking.rebuild`), 그것을 **실패하는 즉시** 흘리면 전량 실패에서 원문이 통째로
  화면에 나간 뒤 오류로 갈아엎는다 — 답이 나왔다가 사라진다. **구현 중 실제로 그렇게
  만들었고 스모크가 잡았다.** 지금은 한 글자도 안 흘린 실패 조각을 만나면 그 자리에서
  멈추고, `gather` 가 끝나 `ok` 면 풀어 흘린다(전량 실패면 풀지 않는다 — 흘린 것이 0 이라
  오류 프레임만 나간다).
- **`stream_fallback` 을 잃을 뻔했다** — 폴백 결과로 `outcome` 을 덮으면 그 객체의
  `stream_unsupported` 는 거짓이라 "되돌아갔다" 는 사실이 사라진다. 값을 만들어 놓고 잃는
  자리가 이 저장소의 단골이다(2026-08-27 건과 같은 형태).
- **그물**: `SFR-018/tests/test_polish_chunking.py` **+15건**(`PolishStreamOrderTest` 8 +
  `PolishStreamTransportTest` 7), `check_unit_endpoints` 107 → **113**,
  `check_workflow_run` 103 → **108**. 뒤엣것은 **HTTP 경계에 대역을 꽂는다** — `_stub_gateway`
  는 `_post_serving` 만 바꾸므로 그대로 두면 `_stream_polish` 가 실패해 **폴백으로 지나가고
  스트리밍 경로를 한 줄도 태우지 않는다**(MCP 406 이 넉 달을 살아남은 것과 같은 공백이다).
  **다섯 갈래를 각각 되돌려 FAIL 을 확인했다**: 순서 버퍼 · 실패 조각 보류 · `content` 에
  `strip()` · 스텝의 조건부 스트리밍(두 번 흘린다) · Accept 헤더.
- **미검증**: 실환경에서 게이트웨이가 `stream: True` + SSE 를 내주는지, 그리고 **중간 프록시가
  SSE 를 모아 보내지 않는지**(모으면 "한방에" 로만 보이고 오류가 없다 — 그래서
  `X-Accel-Buffering: no` 를 실어 둔다).

### 결과는 **흘리고**, 하이라이트는 끝나고 **갈아 끼운다** (2026-09-01)

좌우 비교 화면이 붙으면서 2026-08-28 에 토큰 스트리밍을 없앴는데, 요구가 바뀌었다 —
**바뀐 쪽이 "AI 가 주루룩 답변하는" 것처럼 보여야 한다.** 그때 적어 둔 근거("전용 UI 가
한 번에 그리므로 필요 없다")는 화면이 **완성된 뒤**를 말한 것이고, 그 전 수십 초 동안
화면이 비어 있다는 사실은 다루지 않았다. **번역·글다듬이 둘에 되살렸다.**

- ~~**FAQ 는 그대로 안 흘린다**~~ — **2026-09-02 에 되살렸다** (요구 변경: 네 기능이 다
  "주루룩" 이어야 한다). 흘리는 것은 서빙이 조립한 마크다운이고 `result` 는 여전히
  `faq_items` 를 낸다. **그런데 그때 그물을 안 옮겼다** — `check_workflow_run` 이 아직
  FAQ 를 `_NO_STREAM_STEPS` 로 분류하고 `_check_faq_contract` 는 `_drain` 으로 토큰을
  버려서, **FAQ 스트리밍은 한 번도 검사된 적이 없다.**
- **흘리는 것은 정본이다.** 사본(`<mark>`)을 흘리면 ① 하이라이트가 스트리밍 중에 이미
  나타나 요구가 말한 순서와 어긋나고 ② 태그가 조각 경계에서 갈려 `<ma` 같은 부스러기가
  남는다. **원시 마크다운·태그가 스트리밍 중에 보이는 것은 허용된 동작이다**(요구 확정).
  - **번역 쪽이 실수하기 쉽다** — 그쪽은 사본이 코드서빙 응답에 **이미 와 있다**
    (글다듬이는 MCP 를 한 번 더 불러야 생긴다). 그래서 두 단위가 다른 것을 흘리기 쉽고,
    그 어긋남은 오류로 드러나지 않는다.
- **흘리는 시점은 "되돌릴 수 없게 된 뒤" 다.** 서빙 결과를 받고, 번역은 **전량 폴백
  판정까지 끝난 뒤**에 흘린다. 그 판정은 응답이 200 이고 본문도 비어 있지 않은데
  **사용자 원문이 그대로 돌아온** 경우를 오류로 세우는 자리라, 그 앞에서 흘리면 원문을
  번역문인 양 뿌린 뒤 오류로 갈아엎게 된다 — 사용자에게는 **답이 나왔다가 사라지는**
  것으로 보인다. **오류 경로에서는 토큰이 한 개도 나가지 않는다.**
- **점검 호출과 겹쳐 돈다.** 점검을 먼저 띄워 두고 그 동안 흘린다. 순서대로 하면
  스트리밍이 순수한 연출이 되고 전체 시간만 늘어난다.
- **조각 크기는 글 길이에 따라 늘린다.** 32자 고정이면 20만 자 문서가 emit 6,250회다 —
  소켓 메시지 수가 글 길이에 비례하면 긴 문서에서 그 자체가 부하다. 총 emit 수에 상한
  (`_STREAM_MAX_EMITS` 400)을 두고 조각을 키운다. **사본이 3벌이라 006 도 함께 고쳤다**
  (`check_deploy_contract` 의 사본 일치 판정이 잡았다). 006 은 답변이 짧아 동작이
  바뀌지 않는다 — 12,800자 미만에서는 조각이 그대로 32자다.
- **그물**: `check_workflow_run` 85 → **91건**. 흘린 토큰을 버리지 않는 `_drain_with_tokens`
  를 만들었다 — 그전에는 `result` 만 남겨서 **"무엇을 흘렸나" 가 검사된 적이 없었다.**
  네 갈래를 되돌려 FAIL 을 봤다: 스트리밍 제거 · 사본 흘리기 · 오류 경로 흘리기 ·
  조각 상한 제거.
  - **판정 순서를 한 번 고쳤다.** 처음에 무손실 판정을 앞에 뒀더니 사본을 흘렸을 때
    그쪽이 먼저 걸려 **"사본을 흘렸다" 판정이 영영 FAIL 할 수 없었다.** 되돌려 보고
    알았고, 사본 판정을 앞으로 옮겼다(진단도 이쪽이 정확하다).
...
- **018 은 `text` 를 싣지 않는다.** 전용 UI 가 결과를 한 번에 그리므로 채팅용 조립
  문자열이 필요 없다. 그때 **토큰 스트리밍도 없앴는데 2026-09-01 에 되살렸다**
  (번역·글다듬이 둘. FAQ 는 그대로다) — 아래 "결과는 흘리고, 하이라이트는 갈아 끼운다" 절.
...
### 프롬프트를 **라이브러리에서 받는다** — 네 단위 전부 (2026-09-03 요구 변경)

톤·문서유형 **목록**은 2026-08-18 부터 프롬프트 라이브러리에서 받고 있었다. 요구가
넓어졌다 — **자주 바뀌는 프롬프트 문장 자체를 전부 그리로 뺀다.** 006 의 항목 매핑
지시가 그 대표이고(코드서빙 안 `.j2` 파일이었다), 고정 골격인 **시스템 프롬프트는 파일로
둬도 된다.** 가이드 §10.5 가 "코드 PR 로 프롬프트 변경" 을 금지사항으로 든 그 자리다.

- **덮어쓰기지 이사가 아니다.** `.j2` 파일은 기본값이자 **폴백**으로 남고, ID 를 준
  이름만 라이브러리가 이긴다. 파일을 지우고 라이브러리만 두면 admin-api 장애가 곧
  기능 정지가 되고, 이관(손으로 옮겨 적는다)에서 프롬프트가 통째로 빠진다.
- **이름 → ID 매핑 하나로 배선한다** (`<단위>_PROMPT_IDS=extract_user=41,...`, JSON 도
  받는다). 이름은 **파일 이름에서 확장자를 뗀 것**이다 — 별도 이름표를 두면 "파일 이름
  ↔ 라이브러리 이름" 대조표가 하나 더 생기고, 어긋나면 **덮어쓰기가 조용히 일어나지
  않는다.** ID 는 코드에 적지 않는다(§10.5).
- **세 갈래가 전부 파일로 떨어진다** — 미설정 · 조회 실패 · **본문 렌더 실패**(관리자가
  변수 이름을 틀리면 `StrictUndefined` 가 그 자리에서 죽는다). 여기서 요청을 세우면
  문구 오타 하나가 기능을 통째로 막는다. **파일도 없을 때만 요청을 세운다** — 지시문
  없는 프롬프트의 결과는 정상 응답처럼 내려간다(기존 규약 그대로).
- **조용히 옛 문구로 돌지 않게** `GET /prompts` 가 이름마다 `source`/`reason` 을 낸다.
  이게 없으면 "ID 를 안 넣었다" 와 "넣었는데 못 읽었다" 가 화면에서 **똑같이 옛 문구**로
  보인다(글다듬이 `policy.source` 와 같은 규약). **본문은 싣지 않는다** — 담으면 그
  경로가 지시문 유출 경로가 된다(3.8절).
- **사본이 4벌**(단위 간 import 금지)이고 **본문까지 같아야 한다.** 갈리면 같은 관리자
  실수가 단위마다 다르게 끝난다 — 한쪽은 파일로 폴백하고 한쪽은 요청이 죽는 식이다.
  `check_deploy_contract.check_prompt_library_copies()` 가 AST 로 대조한다(독스트링 제외).
- **지시문 언어는 전부 한국어다** (2026-09-03). 시스템 프롬프트도 한국어로 쓴다 —
  라이브러리에 올릴 때도 같다(등록 절차: `onprem/docs/SERVING_REGISTRY.md` §2-3).
- **그물**: `SFR-006/tests/test_prompt_library.py` **신규 10건**(덮어쓰기·세 갈래 폴백·
  캐시·미설정과 조회 실패 구분·본문 미노출), `check_api_contract` 46→**50**,
  `check_unit_endpoints` 83→**89**, `check_deploy_contract` 63→**64**.
  **되돌려 FAIL 을 확인했다** — 로더의 라이브러리 분기를 끄면 "라이브러리 본문이 파일을
  덮는다" 가 FAIL 한다.

### 톤 4종·문서유형 5종으로 갈아 끼웠다 (2026-09-03 요구 변경)

| | 그전 | 지금 |
|---|---|---|
| 톤 | 정중함 · 친절함 · **간결 및 보고체**(개조식 `~함/~임`) | **격식·정중**(`polite`) · **친절·안내**(`friendly`) · **명확·간결**(`clear`) · **사실·객관**(`objective`) |
| 자유 선택군 | 메일 · 게시글 · 보도자료 · 공문 | 메일 · 게시글 · **고객발송문구** |
| 톤 고정군 | 채무 사유 · 심사역 의견 · 재산 의견(→report) · 고객발송문구(→polite) | 채무 사유 · 심사역 의견 — **둘 다 사실·객관 고정** |

- **개조식이 없어졌다.** 새 넷은 전부 존댓말이고 "명확·간결" 은 **문장을 줄이는** 톤이지
  종결어미를 바꾸는 톤이 아니다. 그래서 eval `TONE_RULES` 의 `expected_endings` 가 셋에서
  겹치고, **톤을 가르는 것은 `forbidden`** 이 됐다 — 종결어미만 보던 규칙을 그대로 뒀으면
  네 톤이 전부 통과해 그 지표가 아무것도 거르지 못한다.
- **옛 코드를 별칭으로 받는다** (`LEGACY_TONE_ALIASES` = `{"report": "clear"}`). 없으면
  캔버스에 남은 `report` 가 "모르는 톤" 이 되어 **조용히 기본 톤(격식·정중)으로** 떨어진다 —
  `fell_back`·`tone_overridden` 이 막으려는 바로 그 실패다. 사본이 **2벌**(글다듬이 ·
  MCP)이라 한쪽만 고치면 같은 옛 값이 경로에 따라 다른 톤이 된다. 별칭이 가리키는 톤이
  실재하는지, **판정을 실제로 지나는지**까지 `check_tone_policy` 가 본다(표만 맞고
  `resolve_*` 가 안 쓰면 아무 일도 일어나지 않는다).
  **관리자가 같은 이름의 톤을 등록했으면 그쪽이 이긴다** — 별칭은 "지금 표에 없을 때만" 탄다.
- **없어진 문서유형은 별칭을 두지 않았다.** 보도자료·공문·재산 의견은 기존 규약대로
  기본값(메일)으로 떨어진다 — 톤과 달리 **어느 유형으로 옮길지가 자명하지 않고**, 잘못
  옮기면 그 문서의 추가 지시문이 통째로 바뀐다.
- **프롬프트에는 목록을 적지 않는다.** `system.j2` 는 `tone_label`·`tone_instruction` 을
  받아 끼우기만 한다 — 베껴 두면 톤이 바뀔 때 프롬프트만 옛 목록을 들고 있게 되고 그
  어긋남은 결과물의 문체로만 드러난다.
- **사본은 셋 그대로**(MCP `genon_lang_policy` 원본 · 글다듬이 `tone_presets` · eval
...
### FAQ 의 운영 합불 기준이 **PII 하나**였다 — eval (2026-09-07)

근거성에 기준을 두지 않는 판단(재서술이 곧 오답은 아니다)은 그대로다. 그런데 그 결과로
FAQ 스위트의 `targets` 에 남은 것이 `pii_leak_count` 뿐이라 **30개를 요청해 2개가
나와도 `verdict: pass`** 였다. 품질이 무너져도 리포트가 초록불이면 가드레일이 아니다.

**서빙은 이미 답을 들고 있었다.** `FaqResult.as_payload()` 의 `count`·`requested_count`·
`rejected{schema,ungrounded,duplicate}`·`coverage_capped` 는 전부 **결정적 사실**이고
재서술 논쟁과 무관하다 — 계산해 놓고 아무도 채점하지 않던 값이다(값을 만들어 놓고 화면에
안 쓰던 2026-08-27 건과 같은 형태이고, 이번에는 그 소비자가 eval 이다).

- **거는 것은 둘뿐이다.** `yield_rate`(고른 개수 대비 산출)와 **스키마** 기각률.
  스키마는 **우리가 프롬프트로 못박은 계약**이라 문서 성격과 무관하다.
- **`ungrounded`·`duplicate`·커버리지는 보고만 한다.** 앞의 둘은 원천 문서의 성격에
  달렸고, 커버리지 상한은 **비용 손잡이**(`FAQ_MAX_CHUNK_CALLS`)가 정하는 값이다 —
  배포 설정을 품질 불합격으로 세면 상시 빨간불이 되고 사람이 지표를 끈다.
- **후보가 0건이면 기각률은 `None`** 이다. 0.0 을 주면 아무것도 못 만든 실행이 스키마
  기준을 **만점으로 통과**한다. **요청 개수가 없는 통계는 예외**다(`pairs.pair_texts`
  의 원문 부재 규약과 같다) — 조용히 1.0 을 주면 이 지표를 붙인 이유가 사라진다.
- **임계 둘(0.8 · 0.1)은 실측 전 잠정값**이고 `payload.thresholds` 로 덮는다.
  HANDOFF §A-2 의 조각 예산과 같은 성격이라 실물에서 처음 재는 순간 조정 대상이다.
- **`chrf_score` 의 어긋남은 남겨 뒀다** — `role: operational` 인데 `targets` 에 없어
  값이 아무리 낮아도 verdict 에 안 잡힌다. 참조 번역이 있는 입력이 아직 없어 임계를
  정할 근거가 없다. 정본은 `onprem/eval/README.md`.

### 생성 문서 PII 감사는 **MCP 도구**로, 사람이 직접 돌린다 (2026-09-07)

`pii_leak_count` 는 eval 스위트 안에 있어 **평가를 돌릴 때만** 도는데, 요구는 야간·주간
단위로 **생성 문서를 모아 미마스킹 건수를 집계**하는 것이다. 그래서 다섯 번째 MCP 파일
`onprem/mcp/genon_pii_audit.py` 를 만들었다 — **등록이 9번에서 10번이 된다.**

- **스케줄러를 넣지 않는다** (요구 확정). 주기는 운영이 정하고 우리는 **집계만** 낸다.
  기능 응답 경로(payload)에도 붙이지 않는다 — 매 응답마다 도는 판정이 아니고, 응답에
  실으면 그 값이 화면 계약이 되어 나중에 바꿀 때 발이 묶인다.
- **LLM 을 쓰지 않는다.** ① 미마스킹 개인정보를 찾겠다고 그 텍스트를 모델 호출에
  실으면 **감사 자체가 유출 경로**가 된다 — 세려던 사고를 우리가 낸다. ② 주 단위 추세를
  보는 값인데 판정이 흔들리면 건수가 는 건지 모델이 달라진 건지 구분할 수 없다.
  ③ 허용치가 0이라 오탐 비용이 크고, **오탐은 결국 미탐으로 간다**(사람이 지표를 끈다).
  GenOS 가드레일(#315)과는 경쟁이 아니라 **짝**이다 — 마스킹은 적재 층이 하고 이 도구는
  그 층이 빠졌다는 사실을 **산출물 쪽에서** 잡는다.
- **본문 키가 없는 문서를 "유출 0건" 으로 세지 않는다.** 그렇게 세면 키 이름을 한 번
  잘못 준 감사가 **전건 통과**로 나오고, 그 상태는 오류도 경고도 없이 "이번 주도
  깨끗합니다" 로만 보인다 — 이 도구가 잡으려는 것과 정확히 같은 형태다. `unreadable`
  로 드러내고 `scanned` 를 `documents` 와 따로 낸다.
- **유출 목록에 상한을 두지 않는다.** 상한 뒤의 유출은 리포트에서 사라지는데, 감사에서
  그건 미탐과 같다(변경 하이라이트의 `max_items` 를 없앤 것과 같은 근거).
- **검출한 값은 응답에 담지 않는다** (3.8절) — 유형과 자리만. 담으면 감사 리포트가
  유출 경로가 된다. `not_detected`(이름·주소·계좌번호)를 이유와 함께 함께 내서
  `leak_count == 0` 을 "개인정보 없음" 으로 읽지 못하게 한다.
- **판정부는 `eval/eval_mcp/pii_metrics.py` 의 사본이다** (배포 단위 간 import 금지).
  갈리면 같은 문서가 감사 도구와 평가지표에서 **다른 건수**로 나오고 오류로는 드러나지
  않는다 — `check_mcp_tools` 가 두 구현에 **같은 입력을 태워** 대조하고(정적 diff 가
  아니다) **검출기 목록**도 맞춘다. 표본에 없는 유형을 한쪽에만 더하면 동작 대조만으로는
  안 잡히기 때문이다.
...
### 업로드 문서로 **알아서 채운다** (2026-08-31 요구 추가)

템플릿을 고르고 대화로 채우는 흐름에, **채팅 시작 시 문서를 올리면 그 내용으로 빈 항목을
자동으로 채우는** 경로가 붙었다. 사용자가 이미 가진 문서(사업계획서·회의록)를 사내 양식에
옮겨 적는 것이 실제 업무이기 때문이다.

문서는 캔버스 변수 **`genosUploaded`**(전처리기 산출물)로 온다 — FAQ·번역 스텝 1 이 쓰는
그 경로다. 스텝 1 이 `/chat/context` 뒤에 **`POST /chat/prefill`** 을 이어 부른다
(스텝은 3개 그대로 — 넷으로 만들면 캔버스 등록을 다시 해야 하고, 문서 없는 대화에서는
아무 일도 하지 않는 스텝이 하나 늘어난다).

- **문서를 첫 발화(`question`) 자리에 넣지 않는다.** 처음 떠올리는 방법이고 세 자리에서
  깨진다: ① `chat_api` 가 발화를 `MAX_MESSAGE_CHARS`(2만 자)에서 **조용히 자른다**
  (`question[:상한]`) — 사업계획서는 그 길이를 넘고 뒷부분 값은 없는 것이 된다.
  ② `extract_system.j2` 는 "이번 턴 사용자가 말한 것" 만 담으라는 지시문이라 문서를 그
  자리에 넣으면 **지움 지시·본문 추가 의도를 문서 문장에서 찾아내려 든다.**
  ③ 덮어쓰기 방향이 반대다(아래).
- **대화 중간에도 올릴 수 있고, 파일이 여러 번 온다** (2026-09-02 요구 변경 — 아래
  "중간에 올려도 남은 자리만" 절). 처음 판은 첫 턴 전용이었다.
- **사용자 값을 절대 덮지 않는다** (요구 확정). 빈 항목만 채운다. 두 층으로 막는다 —
  프롬프트에서 채워진 항목을 아예 빼고(`_pending_specs`), 그래도 온 값은 버리고 건수를
  센다(`conflicts`). 프롬프트 지시를 보장으로 보지 않는 §5 규약 그대로다.
  **커밋에서 한 번 더** 본다(값이 HTTP 경계를 건너왔다).
- **같은 턴 발화가 문서를 이긴다.** 커밋이 **문서분을 먼저, 발화분을 나중에** 병합한다.
  순서를 뒤집으면 "이 문서로 채우고 제목은 A 로" 가 문서의 제목으로 되돌아간다.
- ~~**첫 턴에만 태운다**~~ — **2026-09-02 에 걷어냈다** (아래 절). 지금 판정은 둘이다:
  **이미 태운 문서면**(세션 `source_doc_hashes` 멤버십) 안 돌고, **빈 항목이 없으면**
  (`no_pending_fields`) 안 돈다. 표식이 없으면 캔버스가 매 턴 같은 `genosUploaded` 를
  실어 올 때 문서를 다시 태워 **사용자가 지운 값을 우리가 되살린다**(오류가 나지 않아
  제보로만 드러난다). 표식은 저장이 덮어쓰기라 **커밋이 매 턴 다시 실어야 한다.**
  값을 하나도 못 뽑은 문서에서는 **표식이 유일한 방어선**이다.
- **긴 문서는 조각으로 나눈다**(`DOC_CHUNK_CHARS` 12,000 · `DOC_MAX_CHUNKS` 20).
  **앞 조각이 이긴다** — 문서 앞쪽(표지·개요)이 값을 정면으로 적어 둔다. 그리고 **항목이
  다 채워지면 남은 조각을 부르지 않는다**: 조각 수가 곧 LLM 비용이 되지 않게 하는 유일한
  장치다(40조각 문서에서 항목이 첫 조각에 다 있으면 호출은 1회다).
- **실패는 오류가 아니다.** 자동 채움이 실패해도 예외를 올리지 않는다 — 사용자가 원래
  하려던 일(대화로 채우기)이 막히면 **템플릿 채우기 자체가 안 되는 것으로** 보인다.
  대신 `prefill_failed` 를 답변에 한 줄 싣는다(조용히 넘기면 "문서를 올렸는데 아무 일도
  일어나지 않았다" 가 된다).
- **값의 진위는 코드가 판정하지 않는다** (요구 확정 — FAQ 의 근거 대조 같은 층을 넣지
  않았다). 항목명 화이트리스트는 이름만 막는다. 그래서 **채운 값을 답변에 전부 나열**한다 —
  006 은 전용 UI 가 없어 **대화가 곧 화면**이고, 그것이 사용자가 잘못된 값을 발견해 그
  자리에서 고칠 수 있는 유일한 수단이다. 건수만 말하면 문서를 열어 하나하나 대조해야 한다.
- **`TEMPLATE_FILL_DOC_PREFILL=0`** 으로 끌 수 있다.
- **그물**: `SFR-006/tests/test_doc_prefill.py` **신규 16건**(조각 분할 무손실·덮어쓰기
  금지·앞 조각 우선·조기 종료·화이트리스트·설정 부재·빈 응답은 실패가 아니다),
  `check_chat_turn` 23 → **34건**(스텝 3개 체인을 태운다 — 채움·발화 우선·표식·재주입
  금지). 네 갈래를 되돌려 FAIL 을 확인했다: 병합 순서 · 해시 전달 · 덮어쓰기 가드 ·
  (점검 자체의) `doc_prefill.llm_call_async` 대역 누락.
  - **마지막 것이 제일 값어치 있었다.** 처음에 대역을 `chat_api` 에만 꽂았더니 자동 채움
    경로가 **진짜 게이트웨이를 부르려 들었고**, 실패가 fail-open 이라 점검이 통과했다 —
    그때 값이 채워져 있던 것은 **발화 추출이 자동 채움용 대본을 먹었기** 때문이다.
    `from .llm import` 로 이름을 복사한 모듈이 늘면 그 모듈도 꽂아야 한다.

### 중간에 올려도 **남은 자리만** 채운다 (2026-09-02 요구 변경)

자동 채움이 **첫 턴 전용**이었다. 요구가 바뀌었다 — **대화 도중 언제든 파일을 올릴 수
있고, 파일이 여러 번 올 수 있다.** 이미 채운 내용은 밀지 않고 **남아 있는 중괄호만**
그 파일로 채운다.

**바꾼 것은 게이트와 표식 보관 방식뿐이다.** "빈 항목만 채운다" 는 원래부터 두 층
(`_pending_specs` 로 프롬프트에서 빼고, 그래도 오면 `conflicts` 로 버린다)이 지고 있었고,
워크플로우 스텝 1 은 턴 번호를 본 적이 없다(`if document:` 뿐). 첫 턴 제한은 **서빙 쪽
게이트 한 줄**(`if state.values or state.blocks:`)이 걸고 있었다.

- **표식이 문자열 하나 → 목록이 됐다** (`source_doc_hashes`, 세션 `_STATE_VERSION` 3).
  하나만 들면 **두 번째 문서를 태운 순간 첫 문서를 잊고**, 캔버스가 둘을 계속 실어 올 때
  번갈아 가며 다시 태운다 — 지운 값이 되살아나는 그 결함이 **파일이 둘일 때만** 나타나는
  형태로 남는다. 커밋은 덮는 것이 아니라 **더한다**(`_merged_doc_hashes`).
  - **옛 세션(문자열 하나)은 버리지 않고 흡수한다** — 버리면 배포 시점에 진행 중이던
    대화가 다음 턴에 같은 문서를 다시 태운다(`blocks` 를 흡수하는 것과 같은 규율).
  - **상한 20, 오래된 것부터 버린다.** 없으면 긴 대화에서 세션 페이로드가 단조 증가한다.
    버려진 문서가 다시 오면 자동 채움이 한 번 더 돌 뿐이고 값은 안 밀린다.
  - **본문은 세션에 담지 않는다**(3.8절). 자동 채움은 그 턴에 끝나 뒤에서 볼 일이 없다.
- **빈 항목이 없으면 LLM 을 안 부르고, 그 사실을 답변이 말한다**(`no_pending_fields`).
  **항목을 다 채운 뒤 파일을 올리는 것이 이제 정상 흐름**이라, 조용히 넘기면 "올렸는데
  아무 일도 일어나지 않았다" 가 된다(`prefill_failed` 를 안내하는 것과 같은 이유).
  이 안내가 **한 번만** 나가는 것은 그 턴에 해시를 기록하기 때문이다 — 판정 순서가
  `already_applied` **먼저**라야 성립한다.
- **경계를 건너는 것은 이번 턴 해시 하나**이고 목록은 세션에만 있다. 스텝이 목록을 들고
  다니면 그것이 곧 세션 사본이 되고 두 벌이 갈린다.
- **"무엇이 부족한가" 를 한 함수로 모았다** — `doc_prefill._pending_specs` 가
  `missing_field_names` 를 쓴다. 조건을 두 곳에 적으면 "채울 자리가 없다며 건너뛰는데
  정작 빈 항목이 남은" 상태가 생긴다.

**덤으로 기존 결함 하나를 고쳤다 — 그물이 0건이던 자리다.** 화면 직접 편집
(`session_view.save_state` → `PATCH/DELETE /values`·`PUT /blocks`)이 `save_session` 에
**표식을 안 넘기고 있었다.** 저장이 키 하나 덮어쓰기라 그 순간 표식이 지워지고 **다음 턴에
업로드 문서가 통째로 다시 태워진다** — 사용자가 방금 화면에서 지운 값이 되살아나는
것으로 보이고 오류는 나지 않는다. `EditingContext` 가 표식을 들고 다니게 했다.

**그물**: SFR-006 unittest 48 → **54건**, `check_chat_turn` 34 → **41건**,
`check_api_contract` 45 → **46건**. **다섯 갈래를 각각 되돌려 FAIL 을 확인했다** —
게이트 되살리기 · 해시 덮어쓰기 · `no_pending_fields` 제거 · 옛 세션 흡수 제거 ·
화면 편집의 표식 유실. **세 번째는 예상과 다른 판정이 물었다**: 게이트를 없애도 호출 수는
그대로다(`doc_prefill` 이 `pending` 이 비면 첫 조각에서 멈춘다 — 층이 둘이라는 증거다).
실제로 사라지는 것은 **안내문**이다.
...
### 이관은 **파일이 아니라 사람이 건넌다** (2026-09-01, 2026-09-03 갱신)

폐쇄망에 파일을 넣을 수 없다 — **`final_preprocessor.py` 를 화면에 띄워 놓고 타이핑**한다.

**벤더 절반은 이미 그쪽에 있다.** `genos_files/attach_processor.py` 는 온프레미스에서
긁어온 참조 사본이고 원본은 첨부용 전처리기로 등록돼 있다. 그 사본을 떠서 우리 코드를
...
- **빌드 스크립트를 걷어냈다** (2026-09-03 요구 확정). 그전에는
  `build_final_preprocessor.py` 가 조각 셋을 합쳐 `final_preprocessor.py` 를 만들고,
  `build_transfer_kit.py` 가 주석·docstring 을 걷어낸 타이핑 판본(`transfer/`, 1,783줄)과
  오타 검출 해시표를 만들었다. **보는 파일과 치는 파일이 달랐던 것**이고, 정본을 고치고
  키트 재생성을 빼먹으면 **옛 코드를 타이핑하게 되는데 치는 순간에는 맞아 보였다.**
  지금은 정본·검증물·타이핑 대상이 전부 같은 파일이다.
- **대신 그물 하나가 없어졌다** — "생성물이 원본 셋과 같은가"(AST 대조).
  `check_final_preprocessor` **168 → 155건**. 그 판정이 2026-09-03 에 벤더 참조 사본의
  오타(`split_docuㄱments`)를 실제로 잡은 적이 있으므로, **PART 1 은 되도록 손대지 않고**
  고쳐야 하면 참조 사본과 눈으로 대조한다. PART 2·3 의 그물은 그대로다.
- **가드 한 자리가 외부와 다르다.** 이 파일은 PART 1 을 `try:` 안에 넣는데 그건
  **우리 사정**이다(로컬에 docling 이 없어도 점검이 돌아야 한다). 온프레미스에서는 벤더
  스택이 이미 있고 2,587줄을 통째로 들여쓰는 것은 손으로 할 일이 아니라,
  `_FP_ATTACH_IMPORT_ERROR = None` 두 줄이 그 자리를 메운다. **외부 판본과 다른 유일한
  자리**이고 문서에 그렇게 적었다.
- **등록은 한 번이고 갈리는 것은 받을 확장자다.** `hwpx` 만 걸면 나머지는 사이트의 기존
  첨부용 등록이 맡는다 — **PART 1 이 안 뜨는 설치본에서도 안전하다**(가드가
  `except Exception` 이고 PART 2·3 은 가드 밖이다). 전 확장자를 걸려면 사이트 패키지
  판본을 맞춰야 한다.

### hwpx 적재 결과가 **화면에 뜨지 않았다** — 페이지 자리가 비어서다 (2026-09-03)

hwpx 로 넣은 문서는 GenOS 적재 결과 화면에서 확인할 수 없어 사람이 **API 를 직접 쳐서**
봐야 했다. 원인은 레코드다 — 화면은 벤더 레코드 모양을 전제로 청크를 **페이지로 묶어**
그리는데, 이 경로는 페이지 관련 다섯 필드와 `chunk_bboxes`·`media_files` 를 전부 `None`
으로 뒀다("틀린 페이지 번호는 없는 것보다 나쁘다"). **오류가 아니라 빈 목록**이라 로그
어디에도 안 남는다 — 값을 만들어 놓고 화면에 안 쓰던 2026-08-27 건과 방향만 반대다.

- **페이지 자리에 구역(section)을 넣는다.** 지어내는 것과 다른 이유는 **`page_basis`**
  다 — 값의 출처를 레코드가 말하므로 나중에 "페이지 수가 실물과 다르다" 를 추적할 수
  있다. 구역은 hwpx 가 실제로 갖는 경계이고 `i_section` 과 같은 값이라, 화면의 묶음이
  문서 구조와 일치한다. 0 이나 1 로 뭉개면 그 성질이 사라진다.
- **기준을 벤더와 맞춘다** — `i_page` 는 **1-based**(벤더 pdf `page_no`), `i_chunk_on_page`
  는 **0-based**(벤더 `chunk_index_on_page`). 한 컬렉션에 두 기준이 섞이면 화면이
  한쪽만 맞게 그린다.
- **빈 값도 벤더가 쓰는 것을 쓴다** — `chunk_bboxes="[]"`·`media_files=""`
  (`GenOSVectorMetaBuilder` 가 좌표·미디어를 못 찾았을 때 내는 값). `None` 은 벤더가
  절대 내지 않는 값이라 화면이 그 필드를 읽다 멈춘다.
- **되돌릴 자리는 `_page_fields` 하나**다. `i_section`/`n_section` 은 그대로라 이
  매핑이 사라져도 구역 정보는 안 잃는다.
- **고치는 자리는 `final_preprocessor.py` PART 2 하나다** (2026-09-03 부터 그 파일이
  정본이다 — 재생성할 것이 없다).
- **미검증**: 실제 화면에 뜨는지는 폐쇄망에서 확인해야 한다. 화면이 페이지가 아니라
  **원본 미리보기(PDF 렌더)** 를 요구하는 것이라면 이 값으로는 해결되지 않는다 —
  그때는 레코드가 아니라 미리보기 산출물이 없는 것이 원인이다.
- **그물**: `test_preprocessor_chunking` 의 `VectorRecordTest` 2건(값·구역 추종),
  `check_final_preprocessor` **155건** 통과(실물 5벌 레코드 대조 포함. 2026-09-03 에
  빌드를 걷어내며 생성물 대조 판정 13건이 빠져 168 → 155 가 됐다).

### 지능형을 **걷어냈다** — 합친 단위는 첨부용 + hwpx 다 (2026-09-01, **배제 확정 2026-09-03**)

> **지능형은 앞으로 쓰지 않는다** (2026-09-03 요구 확정). 개선안을 낼 때 선택지에
> 올리지 말 것 — **폐쇄망이라 외부 API 를 못 쓰고**, 품질은 **설치 가능한 라이브러리 +
> 우리 코드**로 깎는다. 지능형은 사이트 설치본 판본에 묶여 이미 한 번
> "hwpx 아닌 전 형식 거부" 로 터졌다. pdf 품질 개선 설계는 `onprem/docs/WIP_pdf_tables.md`
> (PyMuPDF 구조 API — 실물 파일 용량 확인 뒤 착수).


**지능형 경로가 실환경에서 동작하지 않아 통째로 뺐다.** 아래 절은 지능형이 있던
시절의 기록이고, 지금 상태는 이렇다:

| | 그전 (2026-08-31) | 지금 |
|---|---|---|
| 합친 단위 | 첨부용 + 지능형 + hwpx, 9,111줄 | **첨부용 + hwpx**, 6,018줄 |
| pdf·ppt·엑셀·이미지 | 지능형 | **첨부용** |
| 겹치는 최상위 이름 | 24개 (제거 13 / 개명 8 / 보존 3) | **셋** (`DocumentProcessor`·`Document`·`_log`) |
| 첨부용의 가부 | 지능형 절반에도 의존 | **첨부용 적재 하나로 정해진다** |
| pdf 조/항/호 | docling 어댑터 | **langchain 어댑터**(새로 만들었다) |
| `check_final_preprocessor` | 154건 | **155건** (이관 키트·생성물 대조는 2026-09-03 에 빠졌다) |

- **감수한 것은 pdf 표 하나다.** 첨부용 pdf 는 `PyMuPDFLoader` 평문 + 문자 수 분할이라
  격자가 문장으로 풀리고 스캔 PDF 는 빈 청크가 된다 — 이 전처리기를 만든 이유(요구사항
  §5 "표 깨짐")가 pdf 에서 재현되는 것이다. **오류가 나지 않고 적재도 되므로** 그
  사실은 "표를 물어봤는데 답이 이상하다" 로만 드러난다. **실물로 재보지 않았다.**
- **pdf 위계는 살렸고, 그 과정에서 어댑터가 둘이 됐다.** 위계 층(`annotate_outline`·
  `chunk_blocks`)은 원래부터 포맷 중립이라 갈아 끼운 것은 **입구와 출구뿐**이다 —
  첨부용 최상위는 langchain `Document` **목록**을 주고받고 `docx_processor` 는
  `DoclingDocument` 를 받는다. 어댑터를 잘못 걸면 매 요청 실패한 뒤 벤더 청커로 돌아가
  **위계가 영영 안 붙는데 결과는 정상으로 보인다.**
- **되살리려면 둘을 같이 고친다**: `final_preprocessor.py`(지능형 절반 복원 + 겹치는
  24개 처리 + PART 3 라우팅) · `onprem/test/check_final_preprocessor.py`.
  지운 코드는 `git show f7c4aec:onprem/preprocessor/build_final_preprocessor.py`.
- **되돌려 FAIL 을 본 갈래는 넷이다**: 첨부용 최상위에 docling 어댑터 걸기 ·
  페이지를 구역으로 쓰기(**두 판정이 함께** FAIL — 구역 판정과 "페이지를 걸친 항도 그
  조와 한 청크다") · metadata 사본 없애기 · 진입점 `DocumentProcessor` 개명 되돌리기.
  - **두 판정은 처음에 하나만 잡혔다.** 조 경계 판정을 `"제4조" in text` 로 썼는데,
    갈라져 나간 청크에도 `제2장 총칙 > 제4조(적용 범위)` **머리말**이 붙어 그 낱말이
    들어 있었다. 조문 **본문**으로 보도록 고치고 다시 확인했다.
  - **metadata 사본도 처음에는 안 잡혔다.** 값 비교로 썼는데 청크마다 원본 페이지가
    달라 공유해도 통과했다 — **정체(`is not`)로** 보도록 고쳤다.
...
| 등록 방식 | 언제 | 타이핑 |
|---|---|---|
| hwpx 단독 | 확장자별로 매핑을 나눌 수 있을 때 (더 단순하다) | 1,244줄 |
| 벤더 파일에 이어 붙이기 | 한 등록이 모든 확장자를 받아야 할 때 | 1,783줄 + 벤더 1줄 수정 |

**"올린다" 가 아니라 "친다" 다** (2026-09-01 정정) — 아래 "이관은 파일이 아니라 사람이
건넌다" 절.
...
- ~~**손으로 붙이지 않고 생성한다.**~~ **2026-09-03 에 빌드를 걷어냈다** — 보는 파일과
  치는 파일이 달라지는 대가가 더 컸다(위 "이관은 파일이 아니라 사람이 건넌다" 절).
  벤더 참조 사본은 GenOS 가 릴리스마다 갱신하므로(`v.2.2.4 (2026-07-30 Release)`)
  **PART 1 을 손볼 때는 `genos_files/attach_processor.py` 와 눈으로 대조한다** — 그
  어긋남은 오류로 드러나지 않는다.
...
조문 위계는 hwpx 전용이었는데 규정집이 hwpx 로만 오지 않는다. 정본은
`onprem/preprocessor/README.md` "조/항/호를 pdf·docx 에도 매긴다".

**위계 층은 원래부터 포맷 중립이었다** — `annotate_outline`·`chunk_blocks` 는
`Block(kind, text, section)` 만 보고 hwpx 전용인 것은 `parse()` 하나다. 그래서 새로
만든 것은 **어댑터 하나**(`DoclingDocument → Block`)이고, 조 경계 청킹·
`제2장 총칙 > 제5조(목적)` 머리말·표 분할은 그대로 재사용된다.

- **벤더의 청커만 갈아 끼운다.** 두 벤더가 같은 3단이라(`load_documents →
  split_documents → compose_vectors`) 가운데만 바꾸고 **`compose_vectors` 는 벤더
  것을 그대로 쓴다.** 그쪽이 bbox·이미지 업로드·**민감정보 마스킹(#315)**·페이지
  카운트를 붙인다 — 우회하면 그 값들이 조용히 사라지고 레코드는 멀쩡해 보인다.
- **그래서 청크가 출처를 들고 다녀야 한다.** `Block.origin`/`Chunk.origin` 을 더했고
  hwpx 경로에서는 **언제나 비어 있다**(실물 5벌 출력이 `reg_date` 빼고 변경 전후 동일).
  벤더 compose 가 `doc_items[0]` 를 무조건 인덱싱하므로 **빈 출처 하나가 적재 전체를
  IndexError 로 죽인다** — 하나라도 비면 벤더 청커로 돌아간다.
- **회귀 위험이 낮은 이유는 `auto` 판정이다.** hwpx 와 같은 조건(`제N조` 2개 이상)이라
  **조문 문서가 아닌 pdf·docx 산출물은 벤더 청커 그대로다.** `statute` 를 명시하면
  지능형이 받는 전 형식에 걸린다(등록자의 선택이다).
- **다시 밟기 쉬운 것 넷**: 청크 모양이 둘이다(첨부용 recursive=dict / 지능형=
  `DocChunk`) · `headings` 를 채우면 제목이 두 번 실린다 · `section` 을 페이지로 쓰면
  **페이지를 걸친 조가 반으로 갈린다** · 처리기는 요청 사이에 공유되므로 인스턴스에
  요청 상태를 적으면 동시 요청이 서로의 설정을 본다.
- **거는 자리는 pdf·docx 둘뿐이다.** `.hwp` 는 한 줄이면 되지만 요구 범위 밖이라 안 걸었다.
- **pdf 는 가정 위에 있다** — docling 이 `제5조`·`①` 을 문단 텍스트로 내주어야 한다.
  `merge_list_items`(항·호 병합)는 청커 단계라 우리가 그 앞에서 가로채 피해 가지만,
  스캔 PDF 의 OCR 오인식·2단 조판은 남는다. **실물 pdf 규정집이 없어 미검증**이고,
  docx 는 `GenosMsWordDocumentBackend` 네이티브라 hwpx 에 가깝다.
- **그물**: `check_final_preprocessor` 119 → **154건**. docling 대역을 등록 단위 밖에서
  꽂는다 — 실물 docling 을 요구하게 두면 이 층을 보는 점검이 **0건이 된다.** 여섯
  갈래를 각각 되돌려 FAIL 을 봤다(출처 전파 · 글자 없는 항목 · 조 경계 · `auto` 판정 ·
  `headings` · 덮인 자리). 마지막 것은 처음에 `<tr>` 개수로 봤는데 **행 수는 그대로라
  안 잡혔다** — 행별 칸 수를 세도록 고치고 다시 확인했다.
...
- ~~**`not/` 판본은 txt 를 올린다**~~ — **2026-09-09 에 hwpx 로 돌아왔다.** mirror 에
  `lxml` 이 들어와 되쓰기가 되므로 정본과 같다(위 "반입 판본을 `not/` 하나로" 절).
...
- **프롬프트는 프롬프트 라이브러리(ID) → 배포 단위 밖 jinja 파일 순으로 찾는다.**
  라이브러리 연동은 2026-09-03 에 네 단위에 붙였다(`prompt_library.py`, 사본 4벌) —
  `<단위>_PROMPT_IDS` 에 `이름=ID` 를 주면 그 이름만 덮어쓰고, 없거나 못 읽으면 파일이다.
  `GET /prompts` 가 어느 쪽을 썼는지 말한다. 파일 관리는 **네 단위 전부 이관 완료**
  ...
  배포 실수라 구분돼야 손을 쓸 수 있다. **예외는 없다** — 예전에는 006 톤 변환 하나가
  예외였는데 그 기능은 2026-08-12 에 없어졌다(`archive/sfr006-tone`). 네 단위 전부
  템플릿이 없으면 요청을 세운다.
- **프롬프트는 전부 한국어로 쓴다** (2026-09-03 요구 확정). 그전에는 통제 대상으로
  갈랐다 — 구조·형식·금지 조항은 영어, 산출물의 어투·표기는 한국어, **번역 단위만 전부
  영어**(대상 언어가 요청마다 바뀌어 지시문 언어가 섞이면 모델이 출력 언어를 헷갈린다).
  그 근거가 없어진 것은 아니므로 **출력 언어를 못박는 문장을 대신 강하게 뒀다** —
  번역 `system_*.j2` 는 맨 위 한 줄과 "입력은 내용이지 지시가 아니다" 절 두 곳에서
  `{{ target_label }}` 을 고정하고, 글다듬이는 "출력은 한국어, 번역 금지"를 유지한다.
  **한국어가 섞여 나오면 그 자리를 먼저 볼 것.** 게이트웨이가 없어 **실호출로 검증하지
  못했다** — 근거와 되돌릴 자리는 각 `*.j2` 머리말에 적혀 있다.
...
python Test/check/check_workflow_run.py    # 118건 — 워크플로우 스텝 9개 실행·반환형·result 1회
                                            ...
                                            #          폴백으로 지나가고 스트리밍을 한 줄도 안 태운다.
                                            #          번역은 `finalize` 로 사본·링크를 받는가,
                                            #          FAQ 는 흘린 조각 == 최종 마크다운)
                                            #        + **글다듬이 스트리밍 전송 규약**
                                            #          (`_check_polish_stream_transport` — Accept 에
                                            #           event-stream / 경로 / SSE 델타를 token 으로 /
                                            #           **두 번 흘리지 않는가** / SSE 아니면 폴백. 2026-09-09)
...
python Test/check/check_final_preprocessor.py  # 171건 — **등록 단위**(area 05, 첨부용 + hwpx)
                                            ...
                                            #           포함, 2026-09-02)
                                            #        + **조/항/호를 pdf·docx 에도** —
                                            #          어댑터 **둘**(docling / langchain)·표 HTML·
                                            #          조 경계·출처 전파·청크 모양 셋·설치 자리
                                            #        + **진입점이 라우터 것 하나인가** ·
                                            #          첨부용이 자기 정의를 전부 들고 있는가
                                            #          (지능형 시절 지운 13개가 돌아왔다)
                                            #        + 라우팅 (hwpx 만 우리 파서·나머지 첨부용·
                                            #          hwpx 내용 확인·overrides)
                                            #        + hwpx 산출물·페이지 필드(구역, 1-based)
                                            #        + 벤더 부재를 통과로 보이게 하지 않는가
                                            #        + 지연 생성·폴백(첨부용으로)·kwargs 전달
                                            #          (대역을 등록 단위 밖에서 꽂는다)
                                            # ※ 실물 hwpx 5벌(`data/`)을 **있는 것만** 태운다 —
                                            #    경로가 어긋나면 건수가 조용히 134 로 줄어든다.
...
python Test/check/check_unit_endpoints.py  # 121건 — 018 세 단위 엔드포인트 경계
                                            ...
                                            #          상한, 2026-09-03)
                                            #        + **톤 강제가 계약으로 나오는가**
                                            #          (`allowed_tones` ↔ **실제 적용되는 톤 집합**,
                                            #           `forced_tone`(bool) ↔ 표, 강제군은 원소 하나,
                                            #           기본값·리로드 응답 일치, 2026-09-02)
...
python Test/check/check_eval_metrics.py    # 88건 — 미측정을 통과로 세지 않는가 ·
                                            ...
                                            #        도구 표면 ↔ 카탈로그 정합성 ·
                                            #        **PII 마스킹 누락**(절대 건수·체크섬 오탐
                                            #          차단·값 미노출·네 기능 공통, 2026-09-02) ·
                                            #        **표시용 `<mark>` 를 벗기는가**(안 벗기면
                                            #          어미 지표가 미측정으로 빠진다)
```
...
그 91 은 위 합계에 넣지 않는다 — `onprem/` 회귀 기준이 아니라 별개 판본의 것이다.
(**2026-09-09 에 이 판본이 반입 대상이 됐다** — lxml 없는 판본이 아니라 기능 최대
판본이고, 그물은 91건 그대로인데 **구성이 통째로 다르다.** 위 절 참고.)
...
unittest 2벌은 그대로다(SFR-006 64 / SFR-018 300 을 돌려서 확인했다).
되돌려 FAIL 을 본 갈래는 다섯이다 — 후보 0건 기각률을 0.0 으로 · 요청 개수 부재를
만점으로 · 산출률 기준 제거 · PII 검출기 하나 제거(사본 대조) · 본문 키 없는 문서를
0건으로.

**2026-09-03 (전처리기 빌드 제거)** — `final_preprocessor.py` 가 정본이 되면서
`check_final_preprocessor` 168 → **155**(생성물 대조·이관 키트 판정 13건이 빠졌다).
나머지 열두 개와 unittest 2벌은 그대로다.

**2026-09-03 (톤 4종·문서유형 5종)** — `check_tone_policy` 22 → **24**,
`check_eval_metrics` 80 → **81**. SFR-018 unittest 는 300 그대로다. `test_admin_policy`
넷이 **내장 톤 목록을 베끼고 있어서** 함께 깨졌고(`["polite","friendly","report"]`·
`len == 3`), 다시 베끼지 않게 `TONE_PRESETS` 에서 파생시켰다 — 표를 베낀 그물은 표가
바뀔 때 **옛 표를 지키는 쪽으로** 실패한다.

**2026-09-03 (FAQ 총 개수 규약 · hwpx 페이지 필드)** — `check_unit_endpoints` 81 →
**83**, SFR-018 unittest 298 → **300**. 나머지 열두 개와 SFR-006 unittest(54)는 그대로다.
빌드 산출물 셋(`final`·`test`·`transfer`)을 다시 만들었고, 그때 **벤더 참조 사본에 들어간
오타**(`split_docuㄱments` — IDE 에 열어 둔 파일에 실수로 입력)를 `check_final_preprocessor`
의 "첨부용 절반이 원본과 같다" 판정이 잡았다. 되돌리고 재생성해 168/168 로 돌아왔다 —
그 판정이 없었으면 **오타가 등록 단위와 타이핑 자료에 그대로 박혔다.**

**2026-09-01 (지능형 제거)** — `check_final_preprocessor` 154 → **155**. 되돌려 FAIL 을
본 갈래는 넷이고, 그중 **둘은 처음에 안 잡혀 판정을 고쳤다** — 위 "지능형을 걷어냈다" 절.

**2026-09-01 (이관 키트)** — `check_final_preprocessor` 155 → **163**. 실물 hwpx 를 못
찾으면 **142** 로 준다(그전 표에 적혀 있던 149 는 지능형 제거 뒤 낡은 값이었다 — 실측해
고쳤다). 되돌려 FAIL 을 본 갈래는 둘이고, **하나는 처음에 안 잡혀 판정을 고쳤다** —
"스니펫이 생성 쪽과 같은 해시를 낸다" 는 생성물에 줄 끝 공백이 없어 `rstrip` 규칙을
지워도 통과했다. 그 규칙은 **사람이 친 코드**를 위한 것이므로 공백을 실제로 끼워 넣고
보는 판정을 따로 붙였다.

**2026-09-02 (사이트 설치본 판본 어긋남)** — `check_final_preprocessor` 163 → **168**
(실물 hwpx 를 못 찾으면 **147**). 실환경에서 pdf 적재가
`ModuleNotFoundError: …facade.enrichment.page_description` 로 막혔다 — 참조 사본이
그 모듈을 **최상위에서** 들여오는데 사이트 설치본에 없어서 attach 절반이 통째로 가드에
걸리고, 라우터가 **hwpx 아닌 전 형식을 거부했다.** hwpx 는 가드 밖이라 그대로 돌아
**"pdf 만 안 되는"** 얼굴이었다. 그 import 하나를 `try/except ImportError` + 스텁으로
감쌌다(PPT 페이지 설명 전용 심볼이라 pdf 는 지나지 않는다) — 근거는
`final_preprocessor.py` PART 1 의 그 가드 주석.
되돌려 FAIL 을 본 갈래는 둘이고, **하나는 처음에 판정이 사라지는 형태였다** — 기대값을
빌더의 명세에서 가져왔더니 그 명세를 비우면 순회가 0바퀴여서 조용히 통과했다(정확히
결함이 돌아오는 상태다). 점검이 모듈 이름을 직접 들도록 고쳤다.

**2026-09-01 (스트리밍 되살림)** — `check_workflow_run` 85 → **91**. 나머지 열한 개와
unittest 2벌은 그대로다(`check_deploy_contract` 63 / `check_unit_endpoints` 75 /
`check_mcp_tools` 80 / `check_chat_turn` 34 / `check_table_grid` 33 / SFR-018 298 /
SFR-006 48 을 돌려서 확인했다). 되돌려 FAIL 을 본 갈래는 넷이고, **판정 순서를 한 번
고쳤다** — 위 "결과는 흘리고, 하이라이트는 갈아 끼운다" 절.
`check_deploy_contract` 의 **사본 일치 판정이 006 을 잡았다** — `_stream_chunks` 가
3벌인데 조각 상한을 018 둘에만 넣었다.

**2026-08-31 (FAQ 개수 규약)** — 개수를 구간당으로 바꾸면서 세 건수가 움직였다: SFR-018 unittest 290→**298**(`PlanQuotaTest` 재작성 + `PerChunkCountTest` 4건), `check_workflow_run` 84→**85**(구간 축소 안내), `check_unit_endpoints` 74→**75**(개수 상한 두 층 노출). 옛 배분기(총 개수를 조각 수로 나눈다)를 꽂아 새 판정 11건 중 **9건이 FAIL** 하는 것과, 스텝의 안내문을 지워 `check_workflow_run` 이 FAIL 하는 것을 확인했다.
...
**2026-08-31 후반 — 파일 5개를 루트 밖으로 옮겼다** (커밋 `633dc03`, 내용 변경 0):
기술협상서 hwpx 2벌 → `data/`, `전처리기 결과{,_정렬}.md` → `archive/`,
`용어사전.md` → `genos-project/`. **옮긴 것 자체는 무해한데 점검 하나가 조용히 줄었다** —
`check_final_preprocessor._SAMPLES` 가 루트 경로를 들고 있고 **`os.path.exists` 로 거르므로**
없어진 2벌이 오류 없이 빠졌다. 119 → **113건**. 그리고 남은 첫 샘플이 기술협상서에서
`파워.hwpx`(짧다)로 바뀌면서 `chunk_size` 전달 판정이 `1 -> 1` 로 **FAIL** 했다 —
경로를 `data/` 로 고쳐 119건으로 되돌렸다. 실물을 옮길 때는 이 목록을 같이 볼 것.
