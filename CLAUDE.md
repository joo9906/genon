...
- **남은 불일치 두 가지**:
  1. `render()` 는 동기, `load_prompt` 는 async HTTP. 동기 호출부
     (`prompts.build_extract_prompts` 등)가 있어 async 화 또는 캐시가 필요하다.
     캐시를 두면 **프롬프트 리비전을 운영 반영해도 pod 재시작 전까지 옛 본문**을 쓴다 —
     §10.5 가 "코드 수정 없이 문구를 바꾼다"고 말한 이점이 그만큼 깎인다. TTL 로 절충할지
     결정이 필요하다.
