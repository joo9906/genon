"""번역 LLM 호출 런타임 — **`openai` SDK 판본** (`not/openai/`).
...
async def _complete(system_prompt: str, user_text: str) -> LlmResult:
    ...
    for attempt in range(retry_count):
        ...
        try:
            ...
            return LlmResult(content=content.replace("```json", "").replace("```", "").strip(), error_type="")
...
async def llm_call_async(
    ...
    """번역 LLM 호출. 예외를 밖으로 던지지 않고 `LlmResult` 로 반환한다.
...
# 스트리밍 호출 — SDK 판
# ═══════════════════════════════════════════════════════════════════════════
# 위 함수는 다 만들어진 뒤 한 번에 준다. 그러면 화면은 LLM 이 도는 수십 초 동안 비어
# 있고, 그 뒤에 조각내 흘리는 것은 **완성 뒤의 연출**이다.
#
# ## 첫 델타 뒤에는 재시도하지 않는다
#
# 재시도는 **아직 한 글자도 흘리지 않았을 때만** 한다. 델타가 나간 뒤 다시 부르면 그
# 출력이 화면에 **이어붙어** 같은 문장이 두 번 나온다 — 부분 실패보다 나쁘다(사용자는
# 그것을 결과물로 읽는다). 그래서 흘린 뒤의 실패는 그 조각의 실패로 확정한다.
#
# ## 게이트웨이가 스트리밍을 받지 않을 수 있다
#
# **폐쇄망에서 `stream=True` 가 되는지 실물로 확인되지 않았다.** 정본이 지금까지
# `"stream": False` 를 명시해 온 이유가 그것이다. 그래서 이 함수는 그 실패를
# `STREAM_UNSUPPORTED` 로 **갈라서** 돌려주고 호출부가 비스트리밍으로 되돌아간다 —
# 안 가르면 스트리밍을 받지 않는 배포에서 그 기능이 통째로 죽는다.
#
# **SDK 를 쓰면 SSE 파싱이 없어진다.** 정본은 `data: {...}` 줄을 직접 잘라 읽는데
# (`_delta_from_frame`), SDK 가 그 층을 들고 있어 여기서는 이벤트 객체를 받는다.
...
def _delta_text(event: Any) -> str:
    """이벤트 하나에서 증분 텍스트를 꺼낸다.

    **모양이 어긋나면 빈 문자열이다** — 여기서 예외를 올리면 이벤트 하나가 이상하다는
    이유로 잘 흐르던 응답이 통째로 실패한다.
    """
    ...
    if isinstance(delta, dict):
        text = _extract_content(delta.get("content", ""))
        if text:
            return text
    # 일부 게이트웨이는 마지막 프레임에 `message` 를 통째로 넣는다.
...
async def translate_stream_async(system_prompt: str, user_text: str, on_delta) -> LlmResult:
    """스트리밍으로 부르고 델타가 올 때마다 `on_delta` 를 부른다.
    ...
    Returns:
        ...
        `error_type == STREAM_UNSUPPORTED` 면 이 배포는 스트리밍을 받지 않는다.
    """
    ...
    for attempt in range(retry_count):
        ...
        try:
            ...
            if last_upstream_status in _STREAM_REJECT_STATUS and emitted == 0:
                # **스트리밍을 안 받는 배포다.** 호출부가 비스트리밍으로 되돌아간다.
                log_warning(
                    "게이트웨이가 스트리밍을 받지 않는다",
        ...
        if emitted:
            # **이미 흘렸다.** 다시 부르면 화면에 같은 글이 이어붙는다 (위 머리말).
            log_warning(
                "번역 LLM 스트리밍이 도중에 끊겼다 — 재시도하지 않는다",
