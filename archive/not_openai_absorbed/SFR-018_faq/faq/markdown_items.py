...
> 이 파일은 `not/openai/` 판본에만 있다. 정본(`onprem/`)은 응답을 JSON 으로 받아
> `generator._parse_faq_payload` 가 통째로 읽는다.
...
**라벨 상수는 프롬프트(`prompt/SFR-018_faq/stream_system.txt`)와 사본 관계다.** 한쪽만
고치면 모든 항목이 스키마 기각으로 떨어지는데, 그 상태는 "FAQ 가 하나도 안 나온다" 로만
드러난다. `check_openai_units.py` 가 둘을 대조한다.
"""
