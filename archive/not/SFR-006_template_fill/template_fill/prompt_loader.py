...
def prompt_exists(template_name: str) -> bool:
    """이 이름의 프롬프트가 **실제로 있는가** — 라이브러리 본문이든 파일이든.

    템플릿별 프롬프트(`extract_system__보고서`)를 고를 때 쓴다. **이름만 보고 고르면**
    파일이 없을 때 `render` 가 `PromptRenderError` 를 내고, 그러면 **전용 프롬프트를 안
    만든 템플릿이 통째로 죽는다** — 미설정은 정상 경로여야 한다(글다듬이
    `_tone_prompt_name` 이 같은 이유로 같은 모양이다).

    **렌더하지 않는다.** 여기서 렌더하면 변수 없이 부르게 되어 멀쩡한 템플릿이 "없는
    것" 으로 판정된다(변수 누락도 `PromptRenderError` 다).

    Returns:
        라이브러리에 본문이 있거나 이미지에 파일이 있으면 `True`.
    """
    name = _template_stem(template_name)
    if prompt_library.body_for(name) is not None:
        return True
    ...
    if not os.path.isdir(directory):
        return False
    return os.path.isfile(os.path.join(directory, f"{name}{_TEMPLATE_SUFFIX}"))
