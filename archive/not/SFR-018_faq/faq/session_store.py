...
- 다운로드는 세션을 **지우지 않는다** — 같은 FAQ 를 여러 번 내려받을 수 있어야 한다
  (006 은 다운로드가 대화의 끝이라 거기서 종료한다). 형식이 txt 하나가 된 뒤에도
  이 규약은 그대로다: 파일을 잃거나 제목을 바꿔 다시 받는 흐름이 정상이고, 무엇보다
  **다운로드가 세션을 지우면 화면에 남아 있는 FAQ 를 두 번째 클릭에서 못 찾는다.**
