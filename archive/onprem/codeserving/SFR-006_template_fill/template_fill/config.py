...
class Config:
    ...
    #
    # 비워 두면 **다듬지 않고 그대로 넣는다** — 이 단위가 글다듬이 없이도 떠야 한다.
    # 기동 의존이 아니라 호출 시점 의존이다.
    @staticmethod
    def text_polish_serving_id() -> str:
        return os.environ.get("TEXT_POLISH_SERVING_ID", "").strip()

    # 다듬기를 통째로 끈다. 서빙 ID 를 지우는 것과 같지만, **왜 꺼졌는지**가 갈린다 —
    # "안 쓰기로 했다" 와 "배선을 빠뜨렸다" 를 로그에서 구분할 수 있어야 한다.
    POLISH_BLOCKS = os.environ.get("TEMPLATE_FILL_POLISH_BLOCKS", "1").strip() not in {
        "0", "false", "off",
    }

    # 요청 경로에 걸리는 호출이라 짧게 둔다 — 실패하면 원문 그대로 진행한다.
    POLISH_TIMEOUT = float(os.environ.get("TEMPLATE_FILL_POLISH_TIMEOUT", "30"))
    ...
    #
    # **목록에 없는 템플릿은 아래 기본값**으로 떨어진다. 빠뜨린 템플릿에서 다듬기가
    # 통째로 멈추면 "그 템플릿만 문체가 안 맞는" 것보다 나쁘다.
    POLISH_MAP_RAW = os.environ.get("TEMPLATE_FILL_POLISH_MAP", "").strip()
    POLISH_TONE = os.environ.get("TEMPLATE_FILL_POLISH_TONE", "objective").strip()
    POLISH_DOC_TYPE = os.environ.get("TEMPLATE_FILL_POLISH_DOC_TYPE", "").strip()
