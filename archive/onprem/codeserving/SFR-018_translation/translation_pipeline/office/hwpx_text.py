...
(area 05)가 2026-08-19·08-20 에 고친 그 네 자리이고, 이 사본에는 없었다. 요청 경로라
사용자가 결과를 눈으로 본다는 것이 그동안의 근거였는데 **번역에서는 성립하지 않는다** —
여기서 만든 마크다운이 그대로 번역돼 최종 결과물이 되므로, 원문에 있던 글자가 빠져도
대조할 원본이 화면에 없다.
...
HTML 로 내지만(RAG 검색 결과가 프롬프트로 조립될 때 개행이 뭉개져 마크다운 표가 무너진다)
이 경로는 그 조립을 지나지 않으므로 **잃을 것이 없는 표는 마크다운 그대로 둔다**
(`_needs_html`). 그 차이는 `check_table_grid.py` 가 지킨다.

## 경계 (알고 쓰는 한계)

- 셀 안에 또 표가 있으면 마크다운으로는 표현할 수 없어 HTML 로 낸다(`_needs_html`).
...
from ..common.logging_utils import log_warning
...
# 곧 문서에 보이는 글자라 번역 결과에서 그만큼이 조용히 사라졌다.
#
...
#     - 시스템이 문서보안을 해제한다
#
# 이던 것이 번역 입력에서는 앞의 `-` 가 없는 두 문장이 되고, **목록이라는 사실과 항목의
# 층위가 함께 없어진다.** 번역문에도 그 표시가 없으므로 사용자는 원문에 있던 목록을
# 잃은 문서를 받는다.
#
...
# 때문이다.
#
# 이 층의 정본은 전처리기(`onprem/preprocessor/final_preprocessor.py` PART 2)다. 고칠 때는
    ...
    def __init__(self, header_xml: bytes = b"") -> None:
        ...
        if header_xml:
            try:
                ...
                # 머리 정의를 못 읽는 것으로 번역을 막지 않는다 — 번호만 빠진다.
                log_warning(
