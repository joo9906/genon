...
# ── 1:1 정렬 (2026-09-15) ───────────────────────────────────────────────
#
# 글다듬이 시스템 프롬프트가 **"문장을 합치거나 나누거나 새로 만들지 않는다"** 를
# 요구한다. 그 요청이 지켜지면 원문 i 번째 단위와 되쓴 글 i 번째 단위가 **짝**이고,
# 그때는 문장 레벨 diff 를 돌릴 이유가 없다 — 짝이 이미 정해져 있다.
#
# ## 왜 필요한가 — `difflib` 이 못 푸는 두 자리
#
# `SequenceMatcher` 는 **순서 보존(LCS 계열)** 이라 교차하는 짝을 만들 수 없다.
# 그래서 문장이 자리를 옮기면(원문 1번 → 결과 4번) 그 짝은 후보에 오르지도 못하고
# **삭제 + 삽입 두 사건**으로 쪼개진다 — 화면에서는 원문 1번과 결과 4번이 **각각
# 통째로 형광**이 되고, "같은 문장이 옮겨갔다" 는 사실은 어디에도 안 나타난다.
# 1:1 이 서면 `i↔i` 로 고정이라 **짝이 엇갈릴 자리 자체가 없어진다.**
#
# ## 프롬프트를 보장으로 보지 않는다 (§5)
#
...
# ## 문턱 둘을 나눠 둔 이유
#
# 짝 하나가 크게 다시 쓰였다고 **문서 전체를 폴백시키면** 이 경로가 실제로 타는 일이
# 드물어진다(특히 "명확·간결" 톤). 반면 **한 칸 밀림은 거의 모든 짝을 어긋나게** 하므로
...
# 한 짝 안에서 바뀐 낱말이 이 비율을 넘으면 **통째로 한 항목**으로 낸다.
#
# 낱말 단위로 잘게 쪼개는 것은 "어느 낱말을 손질했나" 를 보여주려는 것인데, 문장이
# 크게 다시 쓰이면 조사·흔한 낱말만 `equal` 로 남고 나머지가 흩어져 **형광이 누더기**가
# 된다 — 문장 전체를 칠한 것보다 읽기 나쁘다. 그 지점에서는 "이 문장이 통째로 바뀌었다"
# 가 더 정확한 표시다.
_TGMAX_WORD_CHANGE_RATIO = 0.6


def _TGtrigrams(text: str) -> Counter:
    """문자 3-gram 다중집합. 3자 미만이면 글자 자체를 쓴다.

    `SequenceMatcher.ratio()` 를 쓰지 않는 이유는 비용이다 — 짝마다 O(len²) 이고
    문서 하나에 문장이 수백 개다. `Counter` 교집합은 O(len) 이다(eval `chrF` 를
    O(n²) 에서 고칠 때 쓴 것과 같은 방식).
    """
    body = "".join(text.split())
    if len(body) < 3:
        return Counter(body)
    return Counter(body[i:i + 3] for i in range(len(body) - 2))


def _TGpair_similarity(before: str, after: str) -> float:
    """두 단위가 얼마나 닮았나 — 다중집합 자카드 `[0, 1]`.

    **둘 다 비면 1.0** 이다. 빈 단위끼리는 어긋난 것이 아니라 같은 것이고, 0 을 주면
    빈 줄 하나 때문에 문서 전체가 폴백한다.
    """
    left = _TGtrigrams(before)
    right = _TGtrigrams(after)
    if not left and not right:
        return 1.0
    union = sum((left | right).values())
    if not union:
        return 1.0
    return sum((left & right).values()) / union


def _TGaligned(src_units: list, dst_units: list) -> bool:
    """`i↔i` 로 봐도 되는가 — 단위 수가 같고 어긋난 짝이 드문가.

    **여기서 True 가 나오면 문장 레벨 diff 를 아예 돌리지 않는다.** 그래서 이 판정이
    느슨하면 틀린 짝에 낱말 diff 를 돌리게 되고, 그 결과는 오류가 아니라 **엉뚱한 자리가
    형광인 화면**으로만 드러난다.
    """
    ...
    misaligned = sum(
        1
        for src, dst in zip(src_units, dst_units)
        if _TGpair_similarity(src[0], dst[0]) < _TGMIN_PAIR_SIM
    )
    # **짝 하나는 언제나 봐준다** (`max(1, …)`). 비율만 쓰면 다섯 문장 미만 문서에서
    # 허용치가 1 밑으로 떨어져 **무관용**이 되고, 한 문장만 크게 다시 써도 문서 전체가
    # 폴백한다 — 짧은 글이 흔한 이 기능에서 그러면 1:1 경로가 거의 안 탄다.
    #
    # 이동·밀림은 **거의 모든 짝**을 어긋나게 하므로 이 한 칸으로는 못 빠져나간다.
    return misaligned <= max(1, len(src_units) * _TGMAX_MISALIGNED_RATIO)
...
def _TGword_changes(src_units: list, dst_units: list) -> list:
    ...
    # **크게 다시 쓰인 자리는 통째로 한 항목으로** (2026-09-15).
    #
    # 낱말로 잘게 쪼개는 것은 "어느 낱말을 손질했나" 를 보여주려는 것인데, 문장이
    # 크게 다시 쓰이면 조사·흔한 낱말만 `equal` 로 남고 나머지가 흩어져 **형광이
    # 누더기**가 된다 — 문장 전체를 칠한 것보다 어느 낱말을 고쳤는지가 오히려 묻힌다.
    # 그 지점을 넘으면 "이 구간이 통째로 바뀌었다" 가 더 정확한 표시다.
    #
    # **항목이 둘 이상일 때만 접는다.** 한 건짜리는 이미 그 자체가 통째 표시라
    # 접어도 달라지는 것이 없고, 접으면 앞뒤 안 바뀐 낱말까지 형광에 들어간다.
    total = max(len(before_words), len(after_words))
    changed = max(changed_before, changed_after)
    if len(items) > 1 and total and changed / total > _TGMAX_WORD_CHANGE_RATIO:
        return [_TGchange(
            " ".join(w[0] for w in before_words),
            " ".join(w[0] for w in after_words),
            _TGspan(before_words, 0, len(before_words)),
            _TGspan(after_words, 0, len(after_words)),
        )]
    return items
...
def tgbuild_change_list(original: str, polished: str) -> List[TGChangeItem]:
    ...
    # **1:1 이 서면 문장 레벨 diff 를 아예 돌리지 않는다** (2026-09-15).
    #
    # 글다듬이 프롬프트가 문장을 합치거나 나누지 말라고 요구하므로 대부분 여기로 온다.
    # 짝이 이미 정해져 있어 **이동·밀림이 원천적으로 불가능**하고, 낱말 diff 가 문장
    # 안에 갇힌다. 근거와 문턱은 `_TGaligned` 머리말.
    if _TGaligned(src, dst):
        changes: List[TGChangeItem] = []
        for src_unit, dst_unit in zip(src, dst):
            if src_unit[0] == dst_unit[0]:
                continue
...
# `GENON_DEBUG=0` 으로 끈다. 걷어낼 때는 이 블록과 `_tgdebug_echo` 호출만 지운다.
_TGDEBUG_MAX_VALUE = 300


def _tgdebug_echo(message: str, *, event: str = "", **fields) -> None:
    if (os.environ.get("GENON_DEBUG") or "1").strip().lower() in {"0", "false", "off"}:
        return
    parts = [f"event={event}"] if event else []
    for key, value in fields.items():
        text = str(value)
        if len(text) > _TGDEBUG_MAX_VALUE:
            text = f"{text[:_TGDEBUG_MAX_VALUE]}…(+{len(text) - _TGDEBUG_MAX_VALUE}자)"
        parts.append(f"{key}={text}")
    sys.stderr.write(f"[DEBUG {_TGlog.name}] {message} | {' '.join(parts)}\n")
    sys.stderr.flush()
