...
> ```
> prompt/
>   SFR-006_template_fill/   extract_system.txt extract_user.txt extract_body.txt
>                            document_system.txt document_user.txt
>   SFR-018_text_polish/     system.txt sentence_rule.txt
>   SFR-018_translation/     system_batch.txt  user_batch.txt  glossary_batch.txt
>                            system_single.txt user_single.txt glossary_single.txt
>                            system_stream.txt user_stream.txt glossary_stream.txt
>   SFR-018_faq/             md_system.txt md_user.txt md_retry_shortfall.txt
> ```
>
> **확장자는 `.txt` 다** — 2026-09-07 에 jinja 를 걷어내면서 `{{ 이름 }}` 치환만
> 남았고, `.j2` 는 그때부터 거짓말이라 바꿨다(편집기가 jinja 문법을 제안해
> `{% if %}` 를 적게 만든다. 로더는 그 문법을 만나면 요청을 세운다).
> 이 목록이 실제와 갈리면 **`check_prompt_render` 가 아니라 사람만 헷갈린다** —
> 그 점검은 파일을 직접 찾아 렌더한다.
