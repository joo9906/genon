...
번역 의 프롬프트는 이미지에 함께 넣는 `.j2` 파일이었다. 그러면 문구 한 줄을 고치는 데
**코드 PR → 재빌드 → 재배포**가 필요하고, 가이드 §10.5 가 그걸 금지사항으로 못박아 뒀다
(p.58 표: "코드 PR 로 프롬프트 변경 … 비개발자가 수정하기 어렵다"). 글다듬이 톤·문서유형
톤·문서유형 정책이 2026-08-18 에 먼저 이 경로를 깔았고(그 JSON 문서 방식은 2026-09-07 에 이 모듈로 흡수됐다), 번역 지시문(용어사전 활용·문체 지시)도 같은 성격이다 — 언어나 문체가 늘면 손보게
되는 문장이다.
...
`TRANSLATE_PROMPT_IDS` 하나에 담는다. 두 표기를 다 받는다:

    TRANSLATE_PROMPT_IDS=system_batch=41,user_batch=42
    TRANSLATE_PROMPT_IDS={"system_batch": "41", "user_batch": "42"}
