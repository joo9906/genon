"""스트리밍 번역 — **번역문이 만들어지는 대로 흘린다**.

> **배치 경로(`POST /translate/markdown`)는 그대로 남는다.** 그쪽은 스켈레톤 분해 +
> 배치 JSON 이라 흘릴 것이 없고(원시 `{"id":1,"t":"…"` 이 화면에 보이고, 배치 15개가
> 도는 순서도 문서 순서가 아니다), 대신 **구조 보존을 코드가 보장한다** — 표가 많은
> 문서는 그쪽이 맞다. 이 파일은 "번역도 주루룩 보이게" 라는 요구로 생긴 두 번째 경로다.
...
from .glossary_report import highlight_units, terms_for_batch
...
# 받아 그대로 화면에 쓴다 — 좌표만 주면 스텝이 태그를 끼우게 되고, 그러면 겹침 병합·
# 역순 삽입 규칙이 **워크플로우에 한 벌 더** 생긴다. 한쪽만 고치는 실수는 표가 깨지는
# 형태로만 드러난다. 그래서 사본을 여기서 만든다.
_DOCUMENT_UNIT = "__document__"


def highlight_document(text: str, hits: list, *, span_key: str) -> str:
    """문서 **전체** 문자열에 `<mark>` 을 입힌다 — 유닛이 없는 스트리밍 경로용.

    병합·역순 삽입 규칙은 `glossary_report.highlight_units` 것을 **그대로** 쓴다.
    여기서 다시 구현하면 같은 좌표가 경로에 따라 다르게 칠해진다. 하는 일은 문서를
    유닛 하나로 보고 넘기는 것뿐이다 — `hits[]` 에는 `unit_id` 가 없으므로(문서가
    한 단위다) 넘기기 직전에 같은 키를 붙인다.
    """
    if not text or not hits:
        return text
    keyed = [{**hit, "unit_id": _DOCUMENT_UNIT} for hit in hits]
    return highlight_units(
        {_DOCUMENT_UNIT: text}, keyed, span_key=span_key, applied_only=True
    )[_DOCUMENT_UNIT]
