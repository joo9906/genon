...
006 의 프롬프트는 이미지에 함께 넣는 `.j2` 파일이었다. 그러면 문구 한 줄을 고치는 데
**코드 PR → 재빌드 → 재배포**가 필요하고, 가이드 §10.5 가 그걸 금지사항으로 못박아 뒀다
(p.58 표: "코드 PR 로 프롬프트 변경 … 비개발자가 수정하기 어렵다"). 글다듬이 톤·문서유형
톤·문서유형 정책이 2026-08-18 에 먼저 이 경로를 깔았고(그 JSON 문서 방식은 2026-09-07 에 이 모듈로 흡수됐다), 006 의
**항목 추출·문서 자동 채움 지시문**이 같은 성격이다 — 템플릿이 늘거나 항목 표기가 바뀌면
계속 손보게 되는 문장이다.
...
`TEMPLATE_FILL_PROMPT_IDS` 하나에 담는다. 두 표기를 다 받는다:

    TEMPLATE_FILL_PROMPT_IDS=extract_user=41,document_user=42
    TEMPLATE_FILL_PROMPT_IDS={"extract_user": "41", "document_user": "42"}

이름은 **템플릿 파일 이름에서 확장자를 뗀 것**(`extract_user.j2` → `extract_user`)이다.
...
템플릿 채우기 전체를 멈춘다(자동 채움 실패를 오류로 올리지 않는 것과 같은 판단).
...
# 조회 결과를 이 초만큼 재사용한다. 매 요청 HTTP 를 때리면 대화 한 턴이 admin-api 지연에
