...
> **006 이 2026-09-08 에 들여온 네 번째 사본이다.** 그전에는 006 만 파일을
> `POST /generate` 로 **직접** 내려줬다 — 대화 중간에 바로 받는 흐름이라 링크가 필요
> 없다고 봤기 때문이다. 프론트 계약이 네 기능 모두 `download_url` 로 통일되면서
> 006 도 같은 방식이 됐다. **내용은 018 세 사본과 같아야 한다** —
> `check_unit_endpoints` 가 대조한다.
"""
