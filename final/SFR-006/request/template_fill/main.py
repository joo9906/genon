...
async def prompts() -> dict:
    ...
    **본문은 담지 않는다** — 담으면 이 경로가 지시문 유출 경로가 된다.
    """
    # 조회 자체가 admin-api 왕복이라 이벤트 루프를 막지 않게 스레드로 넘긴다
    # (번역 `_startup` 의 blocking 파일 읽기를 `to_thread` 로 뺀 것과 같은 이유).
    return {"prompts": await asyncio.to_thread(prompt_library.status)}


@app.post("/prompts/reload")
async def reload_prompts(x_admin_token: str | None = Header(None)) -> dict:
    """**관리자** — 프롬프트 라이브러리를 즉시 다시 읽는다 (TTL 을 기다리지 않는다).

    글다듬이 `POST /policies/reload`·번역 `POST /glossary/reload` 와 같은 규약이다.
    관리자 토큰을 요구하는 것은 이 경로가 admin-api 를 때리기 때문이다 — 열어 두면
    누구나 반복 호출로 캐시를 무력화할 수 있다.
    """
