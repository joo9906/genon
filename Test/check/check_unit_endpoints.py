...
def _check_llm_request_body(out: list) -> None:
    ...
    **한 단위에만 남으면 그 단위만 다른 요청을 보낸다.** 게이트웨이가 그 필드를 무시하면
    ...
    for label, (unit, rel) in units.items():
        path = os.path.join(unit_dir(unit), *rel.split("/"))
...
def _check_translation(out: list, probe: dict) -> None:
    ...
    with TestClient(main.app) as c:
        ...
        rows = body.get("prompts") if isinstance(body, dict) else None
        out.append(("번역 프롬프트 출처 조회",
                    r.status_code == 200 and isinstance(rows, list),
                    f"HTTP {r.status_code}"))
        # **본문은 싣지 않는다** — 담으면 이 경로가 지시문 유출 경로가 된다 (3.8절).
        out.append(("번역 프롬프트 본문 미노출",
                    isinstance(rows, list) and all("body" not in row for row in rows),
                    str(rows)[:60]))
    ...
    with TestClient(main.app) as c:
        ...
        try:
            main.prompt_library.body_for = lambda name: (
                "관리자 톤 지시문" if name == "system_polite" else None
            )
            out.append(("글다듬이 톤 전용 프롬프트를 쓴다",
                        main._tone_instruction("polite", _TONES["polite"])
                        == "관리자 톤 지시문",
                        main._tone_instruction("polite", _TONES["polite"])[:40]))
            out.append(("글다듬이 등록 안 된 톤은 내장 지시문으로 떨어진다",
                        main._tone_instruction("objective", _TONES["objective"])
                        == _TONES["objective"].instruction
                        and main._tone_instruction("", _TONES["polite"])
                        == _TONES["polite"].instruction,
                        main._tone_instruction("objective", _TONES["objective"])[:40]))

            # **톤과 문서유형이 한 프롬프트에 함께 들어가는가.** 실제 조립 결과를 본다 —
            # 두 함수가 각각 맞아도 조립에서 빠뜨리면 오류 없이 한쪽만 실린다.
            main.prompt_library.body_for = lambda name: {
                "system_polite": "관리자 톤 지시문",
                "doc_type_email": "관리자 문서유형 형식",
            }.get(name)
            prepared, failed = main._prepare_polish(
                main.PolishRequest(text="테스트 문장입니다.", doc_type="email",
                                   tone="polite")
            )
            assembled = prepared[1] if failed is None else ""
            out.append(("글다듬이 톤·문서유형이 한 프롬프트에 합쳐진다",
                        "관리자 톤 지시문" in assembled
                        and "관리자 문서유형 형식" in assembled,
                        assembled[:60]))
            out.append(("글다듬이 톤을 등록해도 골격이 남는다",
                        "[출력 형식" in assembled and "합치거나 나누거나" in assembled,
                        assembled[:60]))
        finally:
            main.prompt_library.body_for = saved_body_for

        # **본문은 싣지 않는다** — 담으면 이 경로가 지시문 유출 경로가 된다 (3.8절).
        out.append(("글다듬이 프롬프트 본문 미노출",
                    isinstance(rows, list) and all("body" not in row for row in rows),
                    str(rows)[:60]))
...
def _check_faq(out: list, probe: dict) -> None:
    ...
    with TestClient(main.app) as c:
        ...
        body = r.json() if r.status_code == 200 else {}
        rows = body.get("prompts") if isinstance(body, dict) else None
        out.append(("FAQ 프롬프트 출처 조회",
                    r.status_code == 200 and isinstance(rows, list),
                    f"HTTP {r.status_code}"))
        # **본문은 싣지 않는다** — 담으면 이 경로가 지시문 유출 경로가 된다 (3.8절).
        out.append(("FAQ 프롬프트 본문 미노출",
                    isinstance(rows, list) and all("body" not in row for row in rows),
                    str(rows)[:60]))
        ...
        out.append(("총 개수 상한 하나만 노출",
                    ...
                    and "total_max_count" not in body
                    and body.get("default_count") == Config.DEFAULT_FAQ_COUNT,
                    f"max_count={body.get('max_count')} /"
                    f" total_max_count={body.get('total_max_count')}"))

        # 배분은 **총 개수를 지킨다.** 합이 요청과 다르면 사용자가 고른 숫자와 받는
        # 개수가 달라진다(그 상태는 오류가 아니라 "왜 이만큼이지" 로만 드러난다).
        # 호출 수 상한은 개수가 아니라 **비용**을 잡는다 — 둘을 한 손잡이에 묶으면
        # 둘 중 하나를 못 지킨다.
        from faq import chunking as faq_chunking

        plans = {
            (chunks, total, calls): faq_chunking.plan_quota(chunks, total, calls)
            for chunks, total, calls in ((40, 30, 6), (4, 8, 6), (40, 3, 6), (1, 5, 6))
        }
        out.append(("총 개수를 조각들이 나눠 갖는다",
                    all(sum(plan) == key[1] for key, plan in plans.items()),
                    " ".join(f"{key}->{sum(plan)}" for key, plan in plans.items())))
        out.append(("호출 수는 상한이 잡는다",
                    all(sum(1 for v in plan if v) <= min(key[2], key[1])
                        for key, plan in plans.items()),
                    " ".join(f"{key}->{sum(1 for v in plan if v)}calls"
                             for key, plan in plans.items())))
