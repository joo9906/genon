...
from paths import WORKFLOW_DIR as _WORKFLOW, unit_dir  # noqa: E402

_UNIT = unit_dir("SFR-006_template_fill")
...
def main() -> int:
    ...
    # 없으면 추출 프롬프트에서 `[지금까지 수집된 값]` 을 빼는 것 같은 변경이 조용히
    # 통과한다 — 모델은 자기가 뭘 고치는지 모른 채 답하고, 값은 A 로 남는다.
    before = read_session("s1").get("values", {}).get("제 목")
    script.push({"updates": {"제 목": "8월 둘째 주 보고"}})
    _, result, handoff = run_turn(steps, "제목을 8월 둘째 주 보고로 바꿔줘", "s1", "주간보고")

    state = read_session("s1")
    rep.expect(
        state.get("values", {}).get("제 목") == "8월 둘째 주 보고",
        "값을 다시 말하면 덮어써진다 (A → B)",
        f"{before!r} -> {state.get('values', {}).get('제 목')!r}",
    )
    # **모델이 고칠 수 있으려면 지금 값이 프롬프트에 실려 있어야 한다.** 이게 빠지면
    # "바꿔줘" 가 무엇을 바꾸라는 것인지 알 수 없고, 그 실패는 값이 안 바뀌는 것으로만
    # 드러난다(오류는 나지 않는다).
    _, last_user_prompt = script.calls[-1]
    rep.expect(
        before in last_user_prompt,
        "바꾸기 직전 값이 추출 프롬프트에 실린다",
    ...
    # 안내문이 `이전 → 새 값` 을 말한다 — 006 은 전용 UI 가 없어 대화가 곧 화면이다.
    rep.expect(
        before in str(result.get("text") or "")
        and "8월 둘째 주 보고" in str(result.get("text") or ""),
        "안내문이 이전 값과 새 값을 함께 말한다",
        str(result.get("text") or "")[:160],
    )
    # 값이 바뀌어도 본문 블록은 그대로다 (수정이 덤을 지우면 안 된다).
    rep.expect(
        len(state.get("blocks") or []) == 2,
        "값 수정이 본문 블록을 건드리지 않는다",
        state.get("blocks"),
    )
