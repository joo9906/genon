...
_SERVER_PY = os.path.join(EVAL_DIR, "eval_mcp", "server.py")
...
def _check_discrimination(rep: Report) -> None:
    ...
    rep.expect(
        forced["tone_applied"] == "objective"
        and forced["tone_forced_by_policy"]
        and not forced["passed"],
        "톤: 문서유형이 톤을 강제하면 그 톤으로 채점한다",
        f"{forced['tone_applied']} passed={forced['passed']} hits={forced['forbidden_hits']}",
    )
    # 새 톤 둘이 **실제로 갈리는지** — 규칙이 비어 있으면 무엇을 넣어도 통과한다.
    clear_ok = tone_metrics.tone_rule_check("연차는 15일입니다. 결재로 신청합니다.", "clear")
    clear_bad = tone_metrics.tone_rule_check(
        "연차에 대하여는 매우 다양한 기준이 적용되는 바입니다.", "clear"
    )
    rep.expect(
        clear_ok["passed"] and not clear_bad["passed"],
        "톤: 명확·간결이 군더더기를 잡는다",
