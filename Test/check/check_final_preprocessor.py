...
1. **이름이 서로를 덮는다** — 세 덩어리가 다 `DocumentProcessor` 를 정의한다. 진입점이
   엉뚱한 것으로 남으면 라우팅이 통째로 사라지고, `Document` 가 덮이면 PART 1 의 20개
   호출부가 **호출 시점에** 터진다(import 는 통과한다).
...
from paths import DATA_DIR as _DATA_DIR, PREPROC_DIR as _PREPROC, ROOT as _ROOT  # noqa: E402
...
_SAMPLES = [
    ...
    os.path.join(_DATA_DIR, "파워.hwpx"),
    os.path.join(_DATA_DIR, "FAQ_결과.hwpx"),
    os.path.join(_DATA_DIR, "FAQ_템플릿.hwpx"),
]
...
# ---------------------------------------------------------------------------
# 1) 사이트 설치본에 없는 벤더 모듈 가드
# ---------------------------------------------------------------------------


def _guard_module(node) -> str:
    """`try: from X import ... except ImportError:` 의 X. 아니면 빈 문자열.

    사이트 설치본에 없는 벤더 모듈을 감싼 자리를 찾는 유일한 수단이다 — 이름으로
    찾으면 가드를 통째로 지웠을 때 **판정이 조용히 0바퀴**가 된다.
    """
...
def _check_optional_import_guard(rep) -> None:
    """사이트 설치본에 없는 벤더 모듈의 import 가 PART 1 을 죽이지 않는가.

    **실환경에서 실제로 밟은 결함이다** — `genon.preprocessor.facade.enrichment.
    page_description` 이 없는 설치본에서 PART 1 이 통째로 바깥 가드에 걸려 라우터가
    **hwpx 아닌 전 형식을 거부했다.** hwpx 는 가드 밖이라 그대로 돌아 "pdf 만 안 되는"
    얼굴로 나타난다.

    런타임으로는 볼 수 없다(이 환경엔 docling 이 없어 PART 1 이 아예 안 뜬다).
    그래서 **소스와 스텁 자체**를 본다.
    """
    with open(_MERGED, "r", encoding="utf-8") as fh:
        on_disk = fh.read()
    tree = ast.parse(on_disk)

    # **모듈 이름을 여기 직접 적는다.** 파일에서 목록을 읽어 오면 그 목록을 비웠을 때
    # 순회가 0바퀴여서 판정이 조용히 사라진다 — 그때가 정확히 이 결함이 돌아오는
    # 상태다. 사이트 패키지가 이 모듈을 갖게 된 것을 확인해서 가드를 뗄 때는 여기도
    # 같이 지운다.
    for name in ("genon.preprocessor.facade.enrichment.page_description",):
        short = name.rsplit(".", 1)[-1]
        guards = [node for node in ast.walk(tree) if _guard_module(node) == name]
        rep.expect(
            len(guards) == 1,
            f"선택 의존 가드가 살아 있다({short})",
            f"가드 {len(guards)}개 — 지우려면 사이트 설치본에 그 모듈이 있음을 먼저 확인할 것",
        )
        if len(guards) != 1:
            continue

        # 스텁을 따로 태운다. import 가 실패하는 설치본에서 **실제로 실행되는 코드**다.
        stub = ast.Module(body=guards[0].handlers[0].body, type_ignores=[])
        namespace: dict = {}
        exec(compile(ast.fix_missing_locations(stub), "<stub>", "exec"), namespace)  # noqa: S102
        ...
        used = set(re.findall(r"_page_desc_options\.([A-Za-z_][A-Za-z0-9_]*)", on_disk))
        missing = sorted(attr for attr in used if not hasattr(options, attr))
        rep.expect(
            not missing,
            "스텁이 호출부가 읽는 속성을 전부 갖는다",
            f"빠진 것: {missing} — 스텁 클래스에 더할 것",
        )


# ---------------------------------------------------------------------------
# 2) 세 덩어리의 이름이 서로를 덮지 않는가
# ---------------------------------------------------------------------------
...
def _check_overlap_handling(rep) -> None:
    ...
    with open(_MERGED, "r", encoding="utf-8") as fh:
        text = fh.read()
    # 벤더 쪽에서 비켜 준 이름은 `DocumentProcessor` → `AttachDocumentProcessor` 하나다.
    # 표식이 있어야 **참조 사본과 대조할 때** 어디가 우리 손이 간 자리인지 보인다.
    rep.expect(
        text.count("# [병합 개명]") == 1,
        "벤더에서 비켜 준 자리에 표식 주석이 있다",
        f"표식 {text.count('# [병합 개명]')}개",
    )
    rep.expect(
        "# [병합 제거]" not in text,
        "지운 정의가 없다 (지능형이 빠지며 전부 되돌아왔다)",
    )
...
def _check_hwpx_path(module, rep) -> None:
    ...
    for sample in samples:
        ...
        rep.expect(
            bool(got) and all(record.get("text") for record in got),
            f"{name}: 빈 텍스트 없이 레코드가 나온다 ({len(got)} 청크)",
        )
        rep.expect(
        ...
        rep.expect(
            ...
            and isinstance(got[0].get("i_page"), int)
            and got[0]["i_page"] >= 1,
            f"{name}: 페이지 자리가 구역으로 채워져 있다(1-based)",
            f"page_basis={got[0].get('page_basis')!r} i_page={got[0].get('i_page')!r}",
        )
