...
from paths import EVAL_DIR, MCP_DIR, unit_dir  # noqa: E402
# 원본은 MCP 도구 파일 **안**에 있다. MCP 는 파일 하나가 등록 단위라 `tone_presets.py`
# 라는 별도 모듈이 없고, 심볼에는 `LP` 접두어가 붙어 있다(`LPTONE_PRESETS`).
_ORIGIN = os.path.join(MCP_DIR, "genon_lang_policy.py")
_COPY_POLISH = os.path.join(
    unit_dir("SFR-018_text_polish"), "text_polish", "tone_presets.py"
)
_COPY_EVAL = os.path.join(EVAL_DIR, "eval_mcp", "tone_metrics.py")
...
def main() -> int:
    ...
    polish_alias = getattr(copy_polish, "LEGACY_TONE_ALIASES", None)
    rep.expect(
        origin_alias == polish_alias,
        "글다듬이 ↔ 원본 옛 톤 별칭표가 같다",
        f"원본={origin_alias}\n글다듬이={polish_alias}",
    )
    rep.expect(
        all(value in origin_tones for value in origin_alias.values()),
        "별칭이 가리키는 톤이 실재한다",
    ...
    rep.expect(
        ...
        "옛 톤 코드가 두 경로에서 같은 톤으로 풀린다",
        f"MCP={origin.lpresolve_tone('email', 'report')[1]!r} "
        f"글다듬이={copy_polish.resolve_policy('email', 'report')[1]!r}",
    )
