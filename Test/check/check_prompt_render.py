...
from paths import UNITS as _PROMPT_UNITS, prompt_dir, unit_dir  # noqa: E402
...
for _unit in (
    ...
    sys.path.insert(0, unit_dir(_unit))
...
def check_template_syntax(rep: Report) -> None:
    ...
    """
    for unit in sorted(_PROMPT_UNITS):
        # `final/` 에서는 프롬프트가 **기능 폴더 안**에 있다 — 목록을 훑지 않고
        # 배포 단위 넷을 이름으로 돈다. 훑으면 기능 폴더가 하나 빠져도 그 사실이
        # "검사 대상이 줄었다" 로만 나타나고 통과한다.
        dir_path = prompt_dir(unit)
...
def check_text_polish(rep: Report) -> None:
    ...
    # 문장 1:1 규칙 (2026-09-15) — `diff_changes` 의 정렬 경로가 이 요청에 얹혀 있다.
    #
    # **톤마다 갈린다**: "명확·간결"(`clear`)의 지시문이 "한 문장에 한 가지만 담고" 라
    # 문장을 나누라는 뜻이므로 그 톤에서는 뺀다. 안 빼면 넷 중 하나가 제 일을 못 하고,
    # 그 손해는 오류가 아니라 **결과물의 문체로만** 드러난다.
    rep.check(
        "합치거나 나누거나" in system,
        "글다듬이 — 문장 1:1 규칙이 실린다",
        system[-400:],
    ...
    clear_system = render(
        ...
        tone_label="명확·간결",
        tone_instruction="짧고 명확한 문장으로 다듬는다.",
    )
    rep.check(
        "합치거나 나누거나" not in clear_system,
        "글다듬이 — `clear` 톤에는 문장 1:1 규칙을 넣지 않는다",
        "문장을 나누라는 톤 지시문과 정면으로 충돌한다",
    )
    rep.check(
        polish_main._sentence_rule_block("polite").endswith("\n"),
        "글다듬이 — 문장 규칙 블록이 개행으로 끝난다",
        repr(polish_main._sentence_rule_block("polite")),
    )
    ...
    path = unit_dir("SFR-018_text_polish", "main.py")
    with open(path, encoding="utf-8") as handle:
        source = handle.read()
    rep.check(
        "doc_type_block=" in source
        and "doc_type_instruction=_doc_type" not in source
        and "sentence_rule_block=_sentence_rule_block(" in source,
