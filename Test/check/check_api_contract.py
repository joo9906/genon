...
def main() -> int:
    ...
    rep.expect(
        res.status_code == 200 and isinstance(body.get("prompts"), list),
        "GET /prompts (프롬프트 출처)",
        res.text[:120],
    )
    # **본문은 절대 싣지 않는다** — 담으면 이 경로가 지시문 유출 경로가 된다 (3.8절).
    rep.expect(
        all("body" not in row for row in body.get("prompts", [])),
        "GET /prompts 는 프롬프트 본문을 싣지 않는다",
        res.text[:120],
    )
    res = client.post("/prompts/reload")
    rep.expect(res.status_code == 403, "POST /prompts/reload 토큰 없으면 403", res.text[:120])
    res = client.post("/prompts/reload", headers=ADMIN)
    rep.expect(
        res.status_code == 200 and isinstance(res.json().get("prompts"), list),
        "POST /prompts/reload (관리자)",
        res.text[:120],
    )
    ...
    _faq_store = unit_dir("SFR-018_faq", "faq", "file_store.py")
