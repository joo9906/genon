...
def _rel(path: Path) -> str:
    """진단에 싣는 짧은 경로. **`final/` 밖이면 저장소 기준으로 낸다.**
...
class Unit:
    ...
    root: str  # `final/` 기준 상대 경로. **평가지표 MCP 만 예외다** (`base` 참고)
    ...
    for path in _py_files(root):
        ...
        for node in ast.walk(tree):
            ...
            for mod in mods:
                ...
                found.setdefault(key, set()).add(_rel(path))
            ...
            if isinstance(node, ast.Assign):
                for tgt in node.targets:
                    if (
                        ...
                        hits.append(f"{_rel(path)}: {tgt.slice.value} 대입")
                ...
                if isinstance(func, ast.Attribute) and func.attr == "setdefault":
                    if node.args and isinstance(node.args[0], ast.Constant):
                        if node.args[0].value in RESERVED_ENV:
                            hits.append(
                                f"{_rel(path)}: {node.args[0].value} setdefault"
                            )
    if hits:
