...
tests/
  ...
  test_api_errors.py    ApiError → HTTP 응답 (오류 로그 레벨 포함)
  ...
  test_doc_prefill.py   업로드 문서 자동 채움 — 조각 분할·덮어쓰기 금지·앞 조각 우선
  test_prompt_library.py 프롬프트 라이브러리가 파일을 덮는가 · 세 갈래 폴백 · 본문 미노출
...
겹치지 않게 나눈다. 예컨대 채팅 흐름은 `check_chat_turn.py`(41건)가 02 스텝 3개와 03
...
| 없앤 스모크 | 지금 그 자리 |
|---|---|
| `smoke_api.py` | `onprem/test/check_api_contract.py` (50건) |
| `smoke_chat_edit.py`·`smoke_crosspath.py` | `onprem/test/check_chat_turn.py` (41건) |
