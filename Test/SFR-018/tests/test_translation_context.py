...
final_path.install(final_path.TRANSLATION_UNIT)
...
class BatchPromptContextTest(unittest.TestCase):
    ...
    def test_output_schema_is_unchanged(self):
        ...
        # 출력 스키마의 **키**(`id`·`t`)가 계약이다 — 설명 문구는 2026-09-03 에
        # 한국어로 바뀌었고, 키가 바뀌면 파서가 통째로 깨진다.
        self.assertIn('{"id": <정수>, "t": "<번역문>"}', system)
        self.assertIn("`c` 는 **번역하지도, 출력하지도", system)
...
class SinglePromptContextTest(unittest.TestCase):
    ...
    def test_system_prompt_warns_about_the_context_line(self):
        ...
        self.assertIn("CONTEXT 줄로 시작할 수 있습니다", system)
