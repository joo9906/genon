...
| 환경변수 | 값 |
|---|---|
| `GENOS_ADMIN_API_URL` | 네 단위 공통 (글다듬이·MCP 는 정책 조회용으로 이미 쓰던 값) |
...
- **세 갈래가 전부 파일로 떨어진다** — 미설정 · 조회 실패 · **본문 렌더 실패**(관리자가
  변수 이름을 틀리면 `StrictUndefined` 가 죽는다). 여기서 요청을 세우면 문구 오타 하나가
  기능을 통째로 막는다. **파일도 없을 때만** 요청을 세운다(아래 규약 그대로).
- **`GET {단위}/prompts`** 가 이름마다 `source`(`prompt_library`/`file`)·`reason` 을 낸다.
  이게 없으면 "ID 를 안 넣었다" 와 "넣었는데 못 읽었다" 가 **똑같이 옛 문구**로 보인다.
...
- 어느 함수를 고치는지는 [`prompt/README.md`](prompt/README.md), 등록 절차는
  [`docs/SERVING_REGISTRY.md`](docs/SERVING_REGISTRY.md) §2-3.
...
- **사용자가 고르는 것은 문서 하나의 총 개수다** (2026-09-03 요구 확정). 어느 구간에서
  몇 개씩 뽑을지는 **우리가 배분한다**(`chunking.plan_quota`) — 고른 숫자가 곧 받는
  개수다. 2026-08-31~09-02 에는 이 값이 구간당 개수여서 구간이 여섯이면 5를 골라도
  30개가 나왔다.
- 배포 상한 `FAQ_MAX_COUNT`(**기본 30** — 옛 총량 상한과 같은 값이라 실제로 만들 수
  있던 최대치가 유지된다), 기본값 `FAQ_DEFAULT_COUNT`(기본 5).
...
- **호출 수는 `FAQ_MAX_CHUNK_CALLS`(기본 6)가 잡는다 — 개수가 아니라 비용의 손잡이다.**
  ...
  있으면 `coverage_capped` 로 알린다. 옛 `FAQ_MAX_TOTAL_COUNT` 는 없어졌다(사용자
  선택이 곧 총 개수라 언제나 안 걸리는 상한이 된다).
...
> 나눈다(나머지는 한 개씩 얹는다).
>
> 2026-08-31~09-02 에는 선택이 **구간당** 개수였다(총량은 `FAQ_MAX_TOTAL_COUNT`).
> 긴 문서에서 구간당 몫이 0 에 가까워지는 것을 막으려던 것인데, 대신 **고른 숫자와
> 받는 개수가 달라졌다**(구간이 여섯이면 5를 골라도 30개). 그 결함은 이제 **호출 수
> 상한**이 막는다 — 태울 구간 수가 묶여 있으므로 조각이 40개여도 몫은 1 밑으로 내려가지
> 않는다. **개수는 사용자가, 비용은 배포가** 정한다.
>
> 상한에 걸려 못 태운 구간이 있으면 **`coverage_capped`** 로 낸다(조각 수 상한인
> `source_truncated` 와 다른 사건이다 — 그쪽은 문서 뒤를 아예 안 봤고, 이쪽은 전체를
> 나눴지만 일부만 태웠다). `/config` 는 이제 상한을 **하나만** 낸다(`max_count`) —
> 옛 `total_max_count` 는 없어졌다.
...
| 5   | `common/prompt_library.py` → `common/prompt_loader.py` → `common/prompt_builder.py` | 순서 고정 (뒤가 앞을 import)        |
...
~~**가이드와 의도적으로 다른 것 하나**~~ — **2026-09-03 에 맞췄다.** §H 가 요구하는
Prompt 리소스(admin-api `GET /prompt/template/{id}`) 경로를 네 단위에 붙였다
