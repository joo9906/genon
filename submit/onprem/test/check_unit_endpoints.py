...
def _check_text_polish(out: list, probe: dict) -> None:
    ...
    with TestClient(main.app) as c:
        ...
        # 그것을 쓰고, 없으면 `system.j2` + `tone_instruction` 으로 떨어진다.
        ...
        try:
            main.prompt_library.body_for = lambda name: (
                "톤 전용 프롬프트" if name == "system_polite" else None
            )
            out.append(("글다듬이 톤 전용 프롬프트를 고른다",
                        main._tone_prompt_name("polite") == "system_polite",
                        main._tone_prompt_name("polite")))
            out.append(("글다듬이 등록 안 된 톤은 system 으로 떨어진다",
                        main._tone_prompt_name("objective") == "system"
                        and main._tone_prompt_name("") == "system",
                        main._tone_prompt_name("objective")))
        finally:
