...
def check_prompt_library_copies(rep: Report) -> None:
    ...
    **본문까지 본다**(로깅 사본은 이름만 본다). 여기서 같아야 하는 것은 함수 집합이
    아니라 **판정**이기 때문이다 — 빈 본문을 거절하는가, 조회 실패에 어떤 사유를 붙이는가,
    캐시를 언제 비우는가.

    독스트링은 비교하지 않는다 — 단위마다 무엇을 라이브러리로 빼는지 설명이 다르고,
    문구까지 맞추라고 하면 주석을 지우는 쪽으로 도망가게 된다. `Config` 접근자 이름은
    같고(`prompt_ids_raw`·`genos_admin_api_url`) 환경변수 이름만 단위마다 다르다.
    """
    units = {
        "006": ONPREM / "codeserving/SFR-006_template_fill/template_fill/prompt_library.py",
        "번역": ONPREM / "codeserving/SFR-018_translation/translation_pipeline/common/prompt_library.py",
        "글다듬이": ONPREM / "codeserving/SFR-018_text_polish/text_polish/prompt_library.py",
        "FAQ": ONPREM / "codeserving/SFR-018_faq/faq/prompt_library.py",
    }
            ...
            if isinstance(node, ast.FunctionDef) and node.body:
                ...
                if isinstance(first, ast.Expr) and isinstance(first.value, ast.Constant):
                    node = ast.fix_missing_locations(
                        ast.FunctionDef(
                            name=node.name, args=node.args, body=node.body[1:],
    ...
    for label, path in units.items():
        if not path.exists():
            rep.add("FAIL", "프롬프트 라이브러리 사본", label, f"파일 없음: {path.name}")
            return
        shapes[label] = normalized(path)

    base_label = "006"
    drifted = [label for label, shape in shapes.items() if shape != shapes[base_label]]
    rep.add(
        "FAIL" if drifted else "OK", "프롬프트 라이브러리 사본", "네 벌이 같은 코드",
        f"{', '.join(drifted)} 가 006 과 다르다 — 같은 관리자 실수가 단위마다 다르게 끝난다"
        if drifted else "006·번역·글다듬이·FAQ 본문 일치",
    )
...
def main() -> int:
    ...
    check_prompt_library_copies(rep)
    check_mcp_files(rep)
