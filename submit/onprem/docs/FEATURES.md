...
**문서유형 5종 × 톤 4종** (2026-09-03 요구 변경 — 그전에는 8종 × 3종).

| | |
|---|---|
| 톤 4종 | `polite` 격식·정중 · `friendly` 친절·안내 · `clear` 명확·간결 · `objective` 사실·객관 |
| 자유 선택군 | 메일 · 게시글 · **고객발송문구**(고정군에서 옮겨 왔다) |
| 톤 고정군 | 채무 및 연체발생 사유 · 심사역 의견 — **사실·객관 고정** |

옛 `report`(간결 및 보고체, 개조식 `~함/~임`)는 없어졌고 새 넷은 **전부 존댓말**이다 —
"명확·간결" 은 문장을 줄이는 톤이지 종결어미를 바꾸는 톤이 아니다. 캔버스에 남은 옛
값은 `LEGACY_TONE_ALIASES`(`report` → `clear`)가 받는다. 없으면 그 선택이 **조용히
기본 톤으로** 바뀐다. 없어진 문서유형(보도자료·공문·재산 의견)은 기존 규약대로 기본
문서유형(메일)으로 떨어진다.
...
- **사용자는 총 개수만 고른다** (2026-09-03 요구 확정). 고른 숫자가 곧 받는 개수이고
  **어느 구간에서 몇 개씩 뽑을지는 코드가 배분한다**(`chunking.plan_quota`). 상한은
  `FAQ_MAX_COUNT`(기본 30) 하나다. 2026-08-31~09-02 에는 선택이 **구간당** 개수여서
  구간이 여섯이면 5를 골라도 30개가 나왔다.
- **호출 수는 `FAQ_MAX_CHUNK_CALLS`(기본 6)가 정한다 — 개수가 아니라 비용의 손잡이다.**
  조각이 40개여도 여섯 조각이 총 개수를 나눠 갖고, 태울 조각은 **고르게 표집한다**
  (앞에서부터 채우면 잘라 쓰던 시절과 결과가 같아진다). 이 상한이 조각당 몫이 0 이
  되는 것도 함께 막는다.
...
**개수 상한과 비용 상한은 다른 손잡이다** (2026-09-03). 위 둘은 사용자가 받을 **총
개수**를 잡고, LLM 호출 수는 `FAQ_MAX_CHUNK_CALLS` 가 잡는다. 하나로 묶으면 둘 중
하나를 못 지킨다 — 옛 `FAQ_MAX_TOTAL_COUNT` 가 그 상태였고, 개수를 지키려다 사용자가
고른 숫자를 구간당으로 바꿔 읽게 됐다.
...
**프롬프트 라이브러리가 파일을 덮어쓴다** (2026-09-03). 자주 손보는 지시문을 재배포
...
지시문 언어는 **전부 한국어**다 (2026-09-03 요구 확정). 그전에는 구조·형식은 영어,
산출물 어투는 한국어로 갈랐고 번역 단위만 전부 영어였다. 대신 **출력 언어를 못박는
문장**을 번역 프롬프트 두 곳에 강하게 뒀다 — 실호출 검증은 못 했다.
