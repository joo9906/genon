...
정책이 2026-08-18 에 먼저 이 경로를 깔았고(`text_polish/policy_store.py`), 글다듬이의 교정 지시문도 같은 성격이다 — 톤·문서유형은 이미 `policy_store` 가 라이브러리
에서 받고 있고, 이 모듈은 **그 값을 끼우는 틀**까지 같은 경로로 옮긴다.
