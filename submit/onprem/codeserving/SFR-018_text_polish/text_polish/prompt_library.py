...
글다듬이 의 프롬프트는 이미지에 함께 넣는 `.j2` 파일이었다. 그러면 문구 한 줄을 고치는 데
**코드 PR → 재빌드 → 재배포**가 필요하고, 가이드 §10.5 가 그걸 금지사항으로 못박아 뒀다
(p.58 표: "코드 PR 로 프롬프트 변경 … 비개발자가 수정하기 어렵다").

톤·문서유형 정책이 2026-08-18 에 먼저 이 경로를 깔았고, **2026-09-07 에 그 JSON 문서
방식이 이 모듈로 흡수됐다** — 톤 전용 프롬프트(`system_<tone>`)와 문서유형 지시문
(`doc_type_<code>`)도 이제 이름=ID 로 여기서 받는다. 글다듬이의 교정 지시문도 같은
성격이다.
...
`POLISH_PROMPT_IDS` 하나에 담는다. 두 표기를 다 받는다:

    POLISH_PROMPT_IDS=system=41
    POLISH_PROMPT_IDS={"system": "41"}
