...
텍스트는 네 기능을 모두 그대로 통과한다 — 기능마다 다르게 재면 안 된다.
...
| `tone_rule_check` | Text | 018 글다듬이 | 톤 프리셋 종결 형태, 금지 표현, 조사 오류(**`nouns` 를 준 자리에서만**) |
> **2026-09-03 — `TONE_RULES` 를 톤 4종으로 갈아 끼웠다.** 새 넷(격식·정중 / 친절·안내 /
> 명확·간결 / 사실·객관)은 **전부 존댓말**이라 `expected_endings` 가 셋에서 겹친다 —
> **톤을 가르는 것은 이제 `forbidden`** 이다. 종결어미만 보던 옛 규칙을 그대로 뒀으면
> 네 톤이 전부 통과해 이 지표가 아무것도 거르지 못한다(개조식 `report` 가 없어졌다).
> `FORCED_TONE_SNAPSHOT` 도 함께 바뀌었다 — 채무 사유·심사역 의견 **둘 다 `objective`**.
> **eval 은 배포 단위를 import 하지 않으므로** 관리자가 톤을 추가하면 여기 규칙이 따라
> 오지 않는다 — 그 톤은 `skipped` 로 드러나고 `tone_pass_rate` 의 **분모에서 빠진다**.
...
민감정보 마스킹(#315)은 **적재 전처리기(첨부용 `guardrail` 모듈)** 가 한다. eval 이
그것을 재는 이유는 그 층이 **없어도 조용하기** 때문이다:

- 사이트 설치본에 그 모듈이 없을 수 있다 (2026-09-02 에 실제로 그 import 가 없어 pdf
  적재가 막혔다 — `onprem/preprocessor/CLAUDE.md`). 설정으로 꺼져 있을 수도 있다.
- 어느 쪽이든 **적재는 성공하고 검색도 된다.** 네 기능은 그 텍스트를 읽어 최종 답변에
  원문 그대로 싣는다. 오류가 없으니 로그에도 안 남는다.

그래서 `pii_leak_count` 는 **네 기능의 최종 답변(`answers`)** 을 본다.

- **비율이 아니라 절대 건수다.** 허용치가 0인 값에 비율 기준을 걸면 200문장 문서의
  1건이 0.5% 로 "거의 완벽" 이 되어 통과한다 — 기준이 사고를 승인한다.
- **`leak_count == 0` 은 "개인정보 없음" 이 아니다.** `detectors` 에 적힌 유형만 본다.
  이름·주소·계좌번호는 검출하지 않는다(형태소·NER 없이 지어낼 수 없고, 계좌번호는
  체크섬이 없어 일반 숫자열과 구분되지 않는다). 그 공백을 응답에 싣는다.
- **`masked_count` 를 함께 센다.** `leak=0 · masked=0` 은 "개인정보가 없던 문서" 와
  "아무것도 안 돌았다" 가 구분되지 않는 상태다.
- **체크섬으로 오탐을 거른다** (주민·사업자·카드). 오탐은 결국 미탐으로 간다 —
  가드레일이 상시 빨간불이면 사람이 임계를 올리거나 지표를 끈다.
- **검출한 값은 응답에 담지 않는다** (3.8절). 자리(`start`/`end`)와 유형만 낸다 —
  담으면 평가 리포트 자체가 유출 경로가 된다.

## 표시용 `<mark>` 는 입력 계약에서 벗긴다 (2026-09-02)

운영 payload 에 남은 텍스트는 하이라이트가 입혀진 **사본**이고 정본은 내려받기 파일에만
있다(2026-08-28). 태그를 그대로 채점하면 종결어미가 `…습니다</mark>` 로 끝나 어미
판정이 전부 `other` 로 떨어지고, `ending_consistency` 가 **불합격이 아니라 미측정으로
조용히 빠진다.**

`pairs.pair_texts` 한 곳에서 벗긴다 — 다섯 집계기가 전부 거기를 지나므로, 각자 벗기게
두면 이 파일이 생긴 이유였던 그 드리프트(집계기마다 다른 키를 읽던 문제)가 되풀이된다.
**`<mark>` 만** 벗기고 `<table>` 같은 원문 구조 태그는 남긴다(지우면 구조 지문이 무너진다).
...
- **한국어 형태소/NER 모델을 쓰지 않는다.** 고유명사는 라틴 대문자 토큰 + 호출부가 준
  목록만 센다 (한계를 결과에 명시).
- **조사 검사는 앞말이 명사임을 아는 자리에서만 한다** (2026-08-30 정정). 받침 유무
  판정 자체는 유니코드 연산으로 정확하지만, **무엇이 조사인지**를 형태소 없이 알 수
  없다. 예전에는 "한글 한 글자 + 조사 글자" 를 전부 조사로 봐서 `평가`·`증가`·`국가`·
  `가을`·`사과` 같은 **평범한 낱말을 오류로 냈고**, 그 결과가 `passed` 에 그대로 들어가
  그런 낱말이 든 문서를 톤 불합격으로 만들었다. `으로/로` 만 보는 절충도 안 된다
  (`진로`·`선로`·`항로`). 지금은 `nouns`(사내 용어사전 표제어 등)를 준 자리에서만 재고,
  검사 여부를 `particle_check.scope` 로 낸다 — **미검사를 통과로 보이게 하지 않는다.**
- **입력 쌍의 키 해석은 `pairs.py` 한 곳에서 한다** (2026-08-30 신설). 다섯 집계기가
  각자 `original`/`source`, `result`/`target` 을 읽어서, 키를 하나로 통일해 주면
  **일부 지표만 빈 문자열을 채점했다** — `polish_structure_pass_rate` 가 `""` 와 `""`
  를 비교해 `pass_rate 1.0` 을 냈다. 원문이 없으면 예외이고, 결과가 없으면 불합격이다.
...
디렉토리에서 한 번 돌려 본 기록이고 저장소에 남지 않았다 — 즉 **네 기능의 합불을 정하는
코드가 자동 점검 없이** 있었다. 가드레일이 조용히 틀리면 틀린 기준으로 합격 도장을
찍으므로, 기능 코드가 틀리는 것보다 나쁘다.

점검은 우선순위 순으로 일곱 층이다:

1. **미측정을 통과로 세지 않는가** (이 패키지의 제1 규약)
2. **아무것도 재지 않고 통과하지 않는가** (입력 계약 — 별칭 키·빈 원문·빈 결과)
3. **판정이 실제로 갈리는가** (통과·불합격 짝을 같이 태운다)
4. **기준 경로가 산출물에 실제로 있는가** — `SUITES[…]["targets"]` 의 경로가 도달하지
   못하면 그 기준은 **영원히 `not_measured`** 다. 완전한 입력을 주고 모든 기준이
   측정되는지 본다 (지표 키를 한 번 바꾸면 조용히 그렇게 된다)
5. **hwpx 지표** — 합성 픽스처(슬롯 2개·표 안 1개 + 누름틀 1개)로 라운드트립·무결성
6. **게이트 규율** — 전건 호출 금지·opt-in·표본 재현성
7. **도구 표면 정합성** — `server.py` 의 `@mcp.tool()` ↔ `catalog.py` ↔ `suites.py`
   (`fastmcp` 없이 `ast` 로 정적으로 읽는다)

**이 점검이 붙자마자 결함 둘이 나왔다**: `tone_pass_rate` 가 운영 합불 기준인데
카탈로그 표에 없었고("표에 없는 도구는 운영 지표가 아니다" 규약과 어긋난 상태),
`_extract_values` 가 **남아 있는 슬롯 표기(`{'제목', 16pt}`)를 값으로 세어** 안 채운
슬롯을 `filled` 로 판정했다 — 부분 초안에서 `agreement_rate`·`guide_state_kept` 가
정확히 반대로 나오던 자리다.
