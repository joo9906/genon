"""내려받는 .txt 의 인코딩·줄바꿈·파일명 규약.

> **`not/` 판본이 006 으로 들여온 네 번째 사본이다** (2026-09-08). 정본 006 은 hwpx 를
> 내려주므로 이 파일이 없다. 여기서는 `lxml` 없이 hwpx 를 되쓸 수 없어 산출물이 txt 가
> 됐고(`document.py` 머리말), 그러면 **018 세 단위와 같은 규약을 써야 한다** — 같은
> 폐쇄망 PC 의 같은 메모장에서 열리는 파일이라 BOM·CRLF 규칙이 갈릴 이유가 없다.
> 내용은 `onprem/codeserving/SFR-018_faq/faq/txt_output.py` 와 **같아야 한다.**

아래는 그 원본의 머리말이다.

## 원래 머리말 (2026-08-12 신규)
