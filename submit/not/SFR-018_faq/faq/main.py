"""FAQ 코드 서빙 진입점 (area 03) — **`lxml` 없는 판본** (`not/`, 2026-09-08).

> **이 파일은 `onprem/codeserving/SFR-018_faq/` 의 한시 판본이다.** 정본은 그쪽이고,
> 사내 PyPI mirror 에 `lxml` 이 들어오면 이 디렉토리는 버린다. 무엇을 왜 뺐는지와
> 되돌리는 법은 `not/README.md`.
>
> **뺀 것은 `POST /generate/upload` 하나다.** 그 라우트만 `faq/hwpx_text.py`(→`lxml`)를
> 썼다. FAQ 의 원문은 어차피 **전처리기 산출물**(캔버스 `genosUploaded`)로 오고
> 워크플로우 스텝 1 은 2026-09-07 부터 그 경로만 쓰므로, **기능 흐름은 그대로다** —
> 없어지는 것은 캔버스를 지나지 않는 **직접 업로드**뿐이다.

엔드포인트
- GET  /health            : 헬스체크 (가이드 필수)
- GET  ""                 : 루트 — 게이트웨이가 경로 없이 베이스를 때리는 경우 대비
- GET  /config            : 관리자 상한·기본 개수·내려받을 수 있는 형식 (UI 가 선택지를 만든다)
- POST /generate          : 마크다운 본문으로 FAQ 생성 (**유일한 입력 경로가 됐다**)
...
# `File`/`Form`/`UploadFile` 을 쓰지 않는다 — 그 셋을 쓰던 라우트가 업로드 하나였다.
# **그래서 `python-multipart` 도 함께 빠진다**: FastAPI 는 `File(...)`/`Form(...)` 을 쓴
...
# `from .hwpx_text import ...` 가 여기 있었다 — 그 모듈이 **모듈 최상단에서 `lxml` 을
# import** 하므로, 업로드 라우트를 안 부르더라도 **기동 단계에서 죽었다.** 지연 import 로
# 미루는 방법도 있지만 그러면 "헬스체크는 통과하는데 기능만 죽은" 상태가 되어 원인이
# 화면에도 로그에도 안 드러난다. 파일째 뺐다.
...
# ─────────────────────────────────────────────────────────────────────────
# `POST /generate/upload` 가 여기 있었다 (hwpx 업로드 직접 파싱). **`lxml` 을 쓰는
# 유일한 라우트**라 이 판본에서 뺐다 — `not/README.md` "되돌리는 법" 참고.
#
# 없어져서 달라지는 것: 캔버스를 지나지 않고 **hwpx 파일을 곧장 이 서빙에 올리는**
# 경로가 없다. 캔버스 흐름(워크플로우 스텝 1 → `genosUploaded` → `POST /generate`)은
# 그대로다. pdf·docx 는 원래도 전처리기를 지나 `/generate` 로 왔다.
# ─────────────────────────────────────────────────────────────────────────
