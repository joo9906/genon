...
# ─────────────────────────────────────────────────────────────
# `read_upload_capped(document, max_bytes)` 가 여기 있었다. 호출부가
# `POST /translate/hwpx` 하나였고 그 라우트를 뺐으므로 **아무도 안 부르는 사본**이
# 된다 — 남겨 두면 `UploadFile` import 가 따라 남고, 나중에 정본과 대조할 때
# "업로드가 되는 줄" 알게 된다. 정본(`onprem/`)에는 그대로 있다.
