...
`sys.path` 에 얹어 `import preprocessor` 로 태운다(등록 단위는 `final_preprocessor.py`
...
| 마크다운 스켈레톤 무손실 왕복 | `check_unit_endpoints`(번역·FAQ 경계), `check_mcp_tools`(MCP 결정적 판정), `check_tone_policy`(톤 3벌 + 별칭 2벌 대조) |
...
| 문서유형 **5종** × 톤 **4종** 정책 판정 | `onprem/mcp/genon_lang_policy.py` — 판정하는 쪽이 원본을 갖는다 (2026-09-03) |
| 구조 훼손 감지·변경 내역 산출 | `onprem/mcp/genon_text_guard.py` (`markdown_guard`·`diff_changes`) |

설계 성질은 그대로다: 변경 내역은 LLM 에 묻지 않고 difflib 으로 **결정적으로** 계산하고,
LLM 호출 결과는 `LlmResult`(전역 오류 상태 없음, 통신/실행 실패를 예외 타입으로 분류)로
돌린다. 톤 프리셋은 **3벌**(글다듬이 · MCP · eval)로 갈려 있고 옛 톤 별칭표가 2벌이다 —
`onprem/test/check_tone_policy.py`(24건)가 둘 다 대조한다.
