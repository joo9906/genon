...
class AdminPolicyTest(unittest.TestCase):
    ...
    def test_builtin_tones_survive(self):
        """**병합이지 대체가 아니다.** 톤 하나를 등록했다고 내장 톤이 사라지면 안 된다.

        내장 목록을 여기 베끼지 않는다 — 톤이 바뀔 때(2026-09-03에 3→4로 바뀌었다)
        이 테스트가 옛 목록을 지키게 된다. `friendly` 만 픽스처가 감춘다.
        """
        self._serve(body=json.dumps(_ADMIN_POLICY, ensure_ascii=False))
        codes = set(t["code"] for t in tone_choices())
        self.assertTrue((set(TONE_PRESETS) - {"friendly"}) <= codes, codes)
        self.assertIn("legal", codes, "관리자 톤이 안 실렸다")
    ...
    def test_builtin_forced_tone_is_not_weakened(self):
        """관리자 톤을 추가해도 내장 강제군은 그대로 강제한다.

        2026-09-03: 강제군은 `debt_reason`·`reviewer_opinion` 둘이고 강제 톤은
        **사실·객관**이다. 고객발송문구는 자유 선택군이 됐으므로 그쪽으로 두면 이
        판정이 강제를 아예 안 보게 된다.
        """
        self._serve(body=json.dumps(_ADMIN_POLICY, ensure_ascii=False))
        self.assertEqual(resolve_policy("debt_reason", "legal")[1:3], ("objective", True))
    ...
    def test_unconfigured_is_not_an_error(self):
        ...
        self.assertEqual(len(tone_choices()), len(TONE_PRESETS))
