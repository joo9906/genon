...
무손실 계약: translator_mode=noop 이면 rebuild 결과가 입력과 바이트 단위로
동일해야 한다 (tests/test_markdown_units.py 에서 검증). 이 계약 덕분에
분해 로직 버그가 "구조 훼손"이 아니라 테스트 실패로 드러난다.
...
def rebuild_markdown(
    ...
    번역이 없는 유닛은 원문 유지 (translate_units_with_mode 폴백 규약과 동일).
    - 번역문의 줄바꿈은 구조(표 행 등)를 깨뜨리므로 공백으로 정규화한다.
    - HTML 텍스트 유닛은 html.escape(quote=False) 로 재이스케이프한다
      (분해 때 unescape 했으므로 왕복 대칭 — noop 무손실 계약의 근거).
    """
